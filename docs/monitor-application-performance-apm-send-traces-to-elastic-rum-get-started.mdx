...
[`apm-server.rum.allow_origins`](((apm-guide-ref))/configuration-rum.html#rum-allow-origins)
...
If you enable the <DocLink id="serverlessObservabilityApmAgentsRumConfigure" section="sendcredentials">`sendCredentials`</DocLink> configuration option, your proxy's response must include the header `Access-Control-Allow-Origin` with the page's origin as a value, and the following header:
...
To learn more about CORS, see the MDN page on
