...
(which is different from the legacy exporter mentioned above) is not intended to be used with Elastic APM and Elastic Observability. Use <DocLink id="serverlessObservabilityApmAgentsOtelNativeSupport" text="Elastic's native OTLP support" /> instead.
