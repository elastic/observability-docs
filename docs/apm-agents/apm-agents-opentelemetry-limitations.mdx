...
Elastic supports both the [(OTLP/gRPC)]({ot_grpc}) and [(OTLP/HTTP)]({ot_http}) protocol with ProtoBuf payload.
Elastic does not yet support JSON Encoding for OTLP/HTTP.
