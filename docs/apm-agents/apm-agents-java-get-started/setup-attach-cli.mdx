...
- Relies on the [runtime attachment capabilities](https://docs.oracle.com/javase/8/docs/jdk/api/attach/spec/com/sun/tools/attach/VirtualMachine.html#attach-java.lang.String-) for java agents of the JVM but doesn't require a full JDK or the `tools.jar`.
- Events that happen while the application starts up may not be captured.
- It is usually not possible to attach to JVMs that are running in containers or in Kubernetes pods using this mechanism. For Kubernetes we support auto-attachment using a [mutating webhook](https://www.elastic.co/guide/en/apm/attacher/current/index.html); for docker you could try the <DocLink id="serverlessObservabilityApmAgentsJavaGetStarted" section="docker">script below</DocLink>

<div id="setup-attach-cli-supported-environments"></div>
...
Additionally, it applies some <DocLink id="serverlessObservabilityApmAgentsJavaConfigure">configuration options</DocLink>.
...
<div id="setup-attach-cli-usage-list"></div>

<div id="setup-attach-cli-usage-options"></div>
...
**-l, --list** 
    ...
    **--no-fork** <DocBadgeAdded tipContent="Added in 1.35.0">1.35.0</DocBadgeAdded> 
    ...
    **-a, --args `<agent_arguments>`** 
    ...
    See <DocLink id="serverlessObservabilityApmAgentsJavaConfigure">Configuration</DocLink> for all available configuration options.
...
<div id="setup-attach-cli-docker"></div>
...
<DocCallOut title="Note">
This script is experimental and might not work with all containers.
Especially the `jq --raw-output .[0].Config.Cmd[0]) == java` might vary.
</DocCallOut>

<p id="attachsh"><strong>attach.sh</strong></p>
...
<div id="setup-attach-cli-troubleshooting"></div>
