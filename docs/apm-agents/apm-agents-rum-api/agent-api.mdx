...
Labels are key/value pairs that are indexed by Elasticsearch and therefore searchable (as opposed to data set via <DocLink id="serverlessObservabilityApmAgentsRumApi" section="apmsetcustomcontext">`apm.setCustomContext()`</DocLink>). You can set multiple labels.
...
* For managed transactions, the agent keeps track of the relevant tasks during the lifetime of the transaction
    ...
    manually by calling the <DocLink id="serverlessObservabilityApmAgentsRumApi" section="transactionend">`end`</DocLink> method.
...
Blocked spans allow users to control the early closing of <DocBadge><DocIcon size="s" type="unlink" title="missing link"/> missing link</DocBadge>{/*  <DocLink id="enApmAgentRumJsCustomTransactions" section="creating-a-managed-transaction">managed transactions</DocLink> */} in few cases when the app contains lots of async activity which cannot be tracked by the agent.
...
See the <DocBadge><DocIcon size="s" type="unlink" title="missing link"/> missing link</DocBadge>{/*  <DocLink id="enApmAgentRumJsCustomTransactionName">custom initial page load transaction names</DocLink> */} documentation for more details.
...
The callback function for these events receives the corresponding transaction object
 ...
 methods and properties documented in <DocLink id="serverlessObservabilityApmAgentsRumApi">Transaction API</DocLink>:
