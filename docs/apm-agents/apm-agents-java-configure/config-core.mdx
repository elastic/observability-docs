...
### `recording` (<DocBadgeAdded tipContent="Added in 1.15.0">1.15.0</DocBadgeAdded>)

<DocCallOut title="Note">
This option was available in older versions through the `active` key. The old key is still 
...
### `enabled` (<DocBadgeAdded tipContent="Added in 1.18.0">1.18.0</DocBadgeAdded>)

Setting to false will completely disable the agent, including instrumentation and remote config polling.
If you want to dynamically change the status of the agent, use <DocLink id="serverlessObservabilityApmAgentsJavaConfigure" section="recording-added[1150]">`recording`</DocLink> instead.
...
### `instrument` (<DocBadgeAdded tipTitle="Added in 1.0.0" tipContent="Changing this value at runtime is possible since version 1.15.0">1.0.0</DocBadgeAdded>)
...
### `service_node_name` (<DocBadgeAdded tipContent="Added in 1.11.0">1.11.0</DocBadgeAdded>)
...
hostname (automatically discovered or manually configured through <DocLink id="serverlessObservabilityApmAgentsJavaConfigure" section="hostname-added[1100]">`hostname`</DocLink>). 

<DocCallOut title="Note">
JVM metrics views rely on aggregations that are based on the service node name. 
...
Similar to the auto-detection of <DocLink id="serverlessObservabilityApmAgentsJavaConfigure" section="service_name">`service_name`</DocLink>, the agent can auto-detect the service version based on the `Implementation-Title` attribute in `META-INF/MANIFEST.MF`.
See <DocLink id="serverlessObservabilityApmAgentsJavaConfigure" section="service_name">`service_name`</DocLink> on how to set this attribute.
...
### `hostname` (<DocBadgeAdded tipContent="Added in 1.10.0">1.10.0</DocBadgeAdded>)
...
<div id="config-long-field-max-length"></div>

### `long_field_max_length` (performance <DocBadgeAdded tipContent="Added in 1.37.0">1.37.0</DocBadgeAdded>)
...
<DocCallOut title="Note">
Review the data captured by Elastic APM carefully to make sure it does not capture sensitive information.
...
<div id="config-enable-instrumentations"></div>

### `enable_instrumentations` (<DocBadgeAdded tipContent="Added in 1.28.0">1.28.0</DocBadgeAdded>)
...
When set to non-empty value, only listed instrumentations will be enabled if they are not disabled through <DocLink id="serverlessObservabilityApmAgentsJavaConfigure" section="disable_instrumentations-added[100changing-this-value-at-runtime-is-possible-since-version-1150]">`disable_instrumentations` (added[1.0.0,Changing this value at runtime is possible since version 1.15.0])</DocLink> or <DocLink id="serverlessObservabilityApmAgentsJavaConfigure" section="enable_experimental_instrumentations-added[1250]">`enable_experimental_instrumentations` (added[1.25.0])</DocLink>.
When not set or empty (default), all instrumentations enabled by default will be enabled unless they are disabled through <DocLink id="serverlessObservabilityApmAgentsJavaConfigure" section="disable_instrumentations-added[100changing-this-value-at-runtime-is-possible-since-version-1150]">`disable_instrumentations` (added[1.0.0,Changing this value at runtime is possible since version 1.15.0])</DocLink> or <DocLink id="serverlessObservabilityApmAgentsJavaConfigure" section="enable_experimental_instrumentations-added[1250]">`enable_experimental_instrumentations` (added[1.25.0])</DocLink>.
...
### `disable_instrumentations` (<DocBadgeAdded tipTitle="Added in 1.0.0" tipContent="Changing this value at runtime is possible since version 1.15.0">1.0.0</DocBadgeAdded>)

A list of instrumentations which should be disabled.
Valid options are `annotations`, `annotations-capture-span`, `annotations-capture-transaction`, `annotations-traced`, `apache-commons-exec`, `apache-httpclient`, `asynchttpclient`, `aws-lambda`, `aws-sdk`, `cassandra`, `concurrent`, `dubbo`, `elasticsearch-restclient`, `exception-handler`, `executor`, `executor-collection`, `experimental`, `finagle-httpclient`, `fork-join`, `grails`, `grpc`, `hibernate-search`, `http-client`, `jakarta-websocket`, `java-ldap`, `javalin`, `javax-websocket`, `jax-rs`, `jax-ws`, `jdbc`, `jdk-httpclient`, `jdk-httpserver`, `jedis`, `jms`, `jsf`, `kafka`, `lettuce`, `log-correlation`, `log-error`, `log-reformatting`, `logging`, `micrometer`, `mongodb`, `mongodb-client`, `okhttp`, `opentelemetry`, `opentelemetry-metrics`, `opentracing`, `process`, `public-api`, `quartz`, `rabbitmq`, `reactor`, `redis`, `redisson`, `render`, `scala-future`, `scheduled`, `servlet-api`, `servlet-api-async`, `servlet-api-dispatch`, `servlet-input-stream`, `servlet-service-name`, `servlet-version`, `sparkjava`, `spring-amqp`, `spring-mvc`, `spring-resttemplate`, `spring-service-name`, `spring-view-render`, `spring-webclient`, `spring-webflux`, `ssl-context`, `struts`, `timer-task`, `urlconnection`, `vertx`, `vertx-web`, `vertx-webclient`, `websocket`.
For version `1.25.0` and later, use <DocLink id="serverlessObservabilityApmAgentsJavaConfigure" section="enable_experimental_instrumentations-added[1250]">`enable_experimental_instrumentations` (added[1.25.0])</DocLink> to enable experimental instrumentations.
...
<div id="config-enable-experimental-instrumentations"></div>

### `enable_experimental_instrumentations` (<DocBadgeAdded tipContent="Added in 1.25.0">1.25.0</DocBadgeAdded>)
...
<div id="config-unnest-exceptions"></div>
...
<div id="config-ignore-exceptions"></div>

### `ignore_exceptions` (<DocBadgeAdded tipContent="Added in 1.11.0">1.11.0</DocBadgeAdded>)
...
<DocCallOut title="Note">
Currently, the body length is limited to 10000 characters and it is not configurable. 
If the body size exceeds the limit, it will be truncated. 
</DocCallOut>

<DocCallOut title="Note">
Currently, only UTF-8 encoded plain text HTTP content types are supported.
The option <DocLink id="serverlessObservabilityApmAgentsJavaConfigure" section="capture_body_content_types-added[150]-performance">`capture_body_content_types` (added[1.5.0] performance)</DocLink> determines which content types are captured.
</DocCallOut>
...
### `global_labels` (<DocBadgeAdded tipTitle="Added in 1.7.0" tipContent="Requires APM Server 7.2+">1.7.0</DocBadgeAdded>)
...
<div id="config-instrument-ancient-bytecode"></div>

### `instrument_ancient_bytecode` (<DocBadgeAdded tipContent="Added in 1.35.0">1.35.0</DocBadgeAdded>)
...
<div id="config-classes-excluded-from-instrumentation"></div>
...
<div id="config-trace-methods"></div>

### `trace_methods` (<DocBadgeAdded tipTitle="Added in 1.0.0" tipContent="Changing this value at runtime is possible since version 1.15.0">1.0.0</DocBadgeAdded>)

A list of methods for which to create a transaction or span.

If you want to monitor a large number of methods,
use  <DocLink id="serverlessObservabilityApmAgentsJavaConfigure" section="profiling_inferred_spans_enabled-added[1150]-experimental">`profiling_inferred_spans_enabled`</DocLink> instead.
...
A few examples:
 ...
 - `org.example.*` <DocBadgeAdded tipTitle="Added in 1.4.0" tipContent="Omitting the method is possible since 1.4.0">1.4.0</DocBadgeAdded>
...
<DocCallOut title="Note">
Only use wildcards if necessary.
The more methods you match the more overhead will be caused by the agent.
Also note that there is a maximum amount of spans per transaction (see <DocLink id="serverlessObservabilityApmAgentsJavaConfigure" section="transaction_max_spans-performance">`transaction_max_spans`</DocLink>).
</DocCallOut>

<DocCallOut title="Note">
The agent will create stack traces for spans which took longer than
<DocLink id="serverlessObservabilityApmAgentsJavaConfigure" section="span_stack_trace_min_duration-performance">`span_stack_trace_min_duration`</DocLink>.
...
<div id="config-trace-methods-duration-threshold"></div>

### `trace_methods_duration_threshold` (<DocBadgeAdded tipContent="Added in 1.7.0">1.7.0</DocBadgeAdded>)

If <DocLink id="serverlessObservabilityApmAgentsJavaConfigure" section="trace_methods-added[100changing-this-value-at-runtime-is-possible-since-version-1150]">`trace_methods`</DocLink> config option is set, provides a threshold to limit spans based on 
...
<DocCallOut title="Note">
Transactions are never discarded, regardless of their duration.
...
<DocCallOut title="Note">
If this option and <DocLink id="serverlessObservabilityApmAgentsJavaConfigure" section="span_min_duration-added[1160]">`span_min_duration`</DocLink> are both configured,
...
### `central_config` (<DocBadgeAdded tipContent="Added in 1.8.0">1.8.0</DocBadgeAdded>)
...
### `breakdown_metrics` (<DocBadgeAdded tipContent="Added in 1.8.0">1.8.0</DocBadgeAdded>)
...
### `config_file` (<DocBadgeAdded tipContent="Added in 1.8.0">1.8.0</DocBadgeAdded>)
...
### `use_elastic_traceparent_header` (<DocBadgeAdded tipContent="Added in 1.14.0">1.14.0</DocBadgeAdded>)
...
<div id="config-disable-outgoing-tracecontext-headers"></div>

### `disable_outgoing_tracecontext_headers` (<DocBadgeAdded tipContent="Added in 1.37.0">1.37.0</DocBadgeAdded>)

Use this option to disable `tracecontext` headers injection to any outgoing communication. 

<DocCallOut title="Note">
Disabling `tracecontext` headers injection means that [distributed tracing](((apm-guide-ref))/apm-distributed-tracing.html) 
...
<div id="config-span-min-duration"></div>

### `span_min_duration` (<DocBadgeAdded tipContent="Added in 1.16.0">1.16.0</DocBadgeAdded>)
...
### `cloud_provider` (<DocBadgeAdded tipContent="Added in 1.21.0">1.21.0</DocBadgeAdded>)
...
<div id="config-enable-public-api-annotation-inheritance"></div>
...
A boolean specifying if the agent should search the class hierarchy for public api annotations (`@CaptureTransaction`, `@CaptureSpan`, `@Traced`).
 ...
 Either way, methods will only be instrumented if they are included in the configured <DocLink id="serverlessObservabilityApmAgentsJavaConfigure" section="application_packages">`application_packages`</DocLink>.
...
<div id="config-transaction-name-groups"></div>

### `transaction_name_groups` (<DocBadgeAdded tipContent="Added in 1.33.0">1.33.0</DocBadgeAdded>)
...
### `trace_continuation_strategy` (<DocBadgeAdded tipContent="Added in 1.34.0">1.34.0</DocBadgeAdded>)
