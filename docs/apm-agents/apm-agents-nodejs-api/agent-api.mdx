...
* `options` `{book['type-object']}` The following options are supported:
    * `name` `{book['type-string']}` Framework name.
    * `version` `{book['type-string']}` Framework version.
    * `overwrite` `{book['type-boolean']}` If set to `false`,
...
* `context` `{book['type-object']}` Accepts the following optional properties:
    * `id` `{book['type-string']}` | `{book['type-number']}` The user's ID.
    * `username` `{book['type-string']}` The user's username.
    * `email` `{book['type-string']}` The user's e-mail.
...
* `context` `{book['type-object']}` Can contain any property that can be JSON encoded.
...
* `value` `{book['type-string']}` | `{book['type-number']}` | `{book['type-boolean']}`
* `stringify` `{book['type-boolean']}`
...
* `labels` `{book['type-object']}` Contains key/value pairs:
    * `name` `{book['type-string']}`
        Any periods (`.`), asterisks (`*`), or double quotation marks (`"`) will be replaced by underscores (`_`),
        as those characters have special meaning in Elasticsearch
...
* `error` - Can be either an `{book['type-error']}` object,
...
* `options` `{book['type-object']}` The following options are supported:
    ...
    * `timestamp` `{book['type-number']}` The time when the error happened.
    ...
    * `message` - If the `error` argument is an `{book['type-error']}` object,
    ...
    * `request` `{book['type-incomingmessage']}` You can associate an error with information about the incoming request to gain additional context such as the request url, headers, and cookies.
    ...
    * `response` `{book['type-serverresponse']}` You can associate an error with information about the http response to get additional details such as status code and headers.
    ...
    * `handled` `{book['type-boolean']}` Adds additional context to the exception to show
    ...
    * `labels` `{book['type-object']}` Add additional context with labels, these labels will be added to the error along with the labels from the current transaction.
    ...
    * `captureAttributes` `{book['type-boolean']}` Whether to include properties on the given `{book['type-error']}` object in the data sent to the APM Server (as `error.exception.attributes`). **Default:** `true`.

    * `skipOutcome` `{book['type-boolean']}` Whether to skip setting the outcome value for the current span to `failure`.  See <DocLink id="serverlessObservabilityApmAgentsNodejsApi" section="spanoutcome">Span outcome</DocLink> for more information. **Default:** `false`.
...
* `name` `{book['type-string']}` The name of the transaction.
...
* `type` `{book['type-string']}` The type of the transaction.
...
* `subtype` `{book['type-string']}` The subtype of the transaction.
...
* `action` `{book['type-string']}` The action of the transaction.
...
* `options` `{book['type-object']}` The following options are supported:
    ...
    * `startTime` `{book['type-number']}` The time when the transaction started.
        Must be a Unix Time Stamp representing the number of milliseconds since January 1, 1970, 00:00:00 UTC.
        Sub-millisecond precision can be achieved using decimals.
        If not provided,
        the current time will be used

    * `childOf` `{book['type-string']}` A W3C trace-context "traceparent" string, typically received from a remote service call.

    * `tracestate` `{book['type-string']}` A W3C trace-context "tracestate" string.

    * `links` `{book['type-array']}` Span links.
...
* `result` `{book['type-string']}` Describes the result of the transaction.
    This is typically the HTTP status code,
    or e.g. "success" or "failure" for a background task

* `endTime` `{book['type-number']}` The time when the transaction ended.
    Must be a Unix Time Stamp representing the number of milliseconds since January 1, 1970, 00:00:00 UTC.
    Sub-millisecond precision can be achieved using decimals.
    If not provided,
    the current time will be used
...
* `name` `{book['type-string']}` Set or overwrite the name of the current transaction.
...
* `name` `{book['type-string']}` The name of the span.
...
* `type` `{book['type-string']}` The type of the span.
...
* `subtype` `{book['type-string']}` The subtype of the span.
...
* `action` `{book['type-string']}` The action of the span.
...
* `options` `{book['type-object']}` The following options are supported:
    ...
    * `startTime` `{book['type-number']}` The time when the span started.
        Must be a Unix Time Stamp representing the number of milliseconds since January 1, 1970, 00:00:00 UTC.
        Sub-millisecond precision can be achieved using decimals.
        If not provided,
        the current time will be used

    * `exitSpan` `{book['type-boolean']}` Make an "exit span".
...
* `err` `{book['type-error']}` the captured exception
...
* `modules` `{book['type-string']}` | `{book['type-string-array']}`
    Name of module(s) to apply the patch to, when required.

* `handler` `{book['type-function']}` | `{book['type-string']}`
    Must be a patch function or a path to a module exporting a patch function

    * `exports` `{book['type-object']}` The original export object of the module
    * `agent` - The agent instance to use in the patch function
    * `options` `{book['type-object']}` The following options are supported:
        * `version` `{book['type-string']}` | `{book['type-undefined']}` The module version, if applicable.
        * `enabled` `{book['type-boolean']}` A flag indicating if the instrumentation is enabled.
