...
_Transaction `subtype` and `action` removed in: v4.0.0_
...
You only need to use this function to end custom transactions created by <DocLink id="serverlessObservabilityApmAgentsNodejsApi" section="apmstarttransaction[name][-type][-options]">`apm.startTransaction()`</DocLink> or if you wish the end a regular transaction prematurely.
