...
* <DocLink id="serverlessObservabilityApmAgentsJavaPerformanceTuning" section="agent-overhead">Agent overhead</DocLink>
* <DocLink id="serverlessObservabilityApmAgentsJavaPerformanceTuning" section="tuning-the-agent-startup">Tuning the Agent Startup</DocLink>
* <DocLink id="serverlessObservabilityApmAgentsJavaPerformanceTuning" section="tuning-the-agent">Tuning the Agent</DocLink>
* <DocLink id="serverlessObservabilityApmAgentsJavaPerformanceTuning" section="circuit-breaker">Circuit Breaker</DocLink>

<div id="agent-overhead"></div>
...
This can be adjusted with the <DocLink id="serverlessObservabilityApmAgentsJavaPerformanceTuning" section="sample-rate">Sample rate</DocLink>.

<div id="tuning-agent-startup"></div>
...
1. Immediate synchronous agent start
    ...
    In this use case, use the `-javaagent` command-line flag as per <DocLink id="serverlessObservabilityApmAgentsJavaGetStarted">Manual setup with `-javaagent` flag</DocLink>
...
1. Fastest start (asynchronously)
    ...
    In this use case you can attach to the application after startup with <DocLink id="serverlessObservabilityApmAgentsJavaGetStarted">Automatic setup with `apm-agent-attach-cli.jar`</DocLink>
...
1. Minimized synchronous start
    ...
    option detailed in the <DocLink id="serverlessObservabilityApmAgentsJavaConfigure">configuration guide</DocLink>. The smallest set of instrumentations
...
<div id="tuning-agent"></div>

## Tuning the Agent

The Java agent offers a variety of <DocLink id="serverlessObservabilityApmAgentsJavaConfigure">configuration options</DocLink>,
...
The sample rate can be changed by altering the <DocLink id="serverlessObservabilityApmAgentsJavaConfigure" section="transaction_sample_rate-performance">`transaction_sample_rate` (performance)</DocLink>.
...
Upping the <DocLink id="serverlessObservabilityApmAgentsJavaConfigure" section="span_stack_trace_min_duration-performance">`span_stack_trace_min_duration` (performance)</DocLink> or disabling stack trace collection altogether can gain you a bit of performance if needed.

### Recording headers and cookies

By default, the Java agent records all request and response headers, including cookies.
Disabling <DocLink id="serverlessObservabilityApmAgentsJavaConfigure" section="capture_headers-performance">`capture_headers` (performance)</DocLink> can save allocations, network bandwidth, and disk space.
...
For fine-grained Circuit Breaker configurations please refer to <DocLink id="serverlessObservabilityApmAgentsJavaConfigure">Circuit-Breaker configuration options</DocLink>.
