...
be sent directly to the Elastic.
...
Elastic supports a ProtoBuf payload via both the OTLP protocol over gRPC transport [(OTLP/gRPC)]({ot_grpc})
...
Other methods, like using the [`elasticsearch` exporter](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/exporter/elasticsearchexporter) to send data directly to ((es)) will send data to Elastic,
but will bypass all of the validation and data processing that the Elastic performs.
...
<DocTable columns={[
  ...
  <DocRow>
    ...
    <DocCell>Elastic URL. The host and port that Elastic listens for events on.</DocCell>
  </DocRow>
  <DocRow>
...
Elastic supports both the [(OTLP/gRPC)]({ot_grpc}) and [(OTLP/HTTP)]({ot_http}) protocol on the same port as Elastic APM agent requests. For ease of setup, we recommend using OTLP/HTTP when proxying or load balancing requests to the Elastic.
