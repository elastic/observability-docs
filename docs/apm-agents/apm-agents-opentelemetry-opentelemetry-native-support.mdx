...
[^6]: Credential for Elastic APM <DocLink id="serverlessObservabilityApmSecretToken">secret token authorization</DocLink> (`Authorization: "Bearer a_secret_token"`) or <DocLink id="serverlessObservabilityApmApiKeys">API key authorization</DocLink> (`Authorization: "ApiKey an_api_key"`).
