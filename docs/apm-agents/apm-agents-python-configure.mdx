---
id: serverlessObservabilityApmAgentsPythonConfigure
slug: /serverless/observability/apm-agents-python-configure
title: Configure
 ...
 * <DocLink id="serverlessObservabilityApmAgentsPythonConfigure" section="central_config">Central configuration</DocLink>
     (supported options are marked with <DocLink id="serverlessObservabilityApmAgentsPythonConfigure" section="dynamic-configuration"><DocBadge>Dynamic</DocBadge> </DocLink>)
...
The Python Agent supports [Central configuration](((apm-app-ref))/agent-configuration.html),
which allows you to fine-tune certain configurations from in the APM app.
This feature is enabled in the Agent by default with <DocLink id="serverlessObservabilityApmAgentsPythonConfigure" section="central_config">`central_config`</DocLink>.

<div id="django-configuration"></div>
...
<div id="flask-configuration"></div>
...
Note: Do not set this if you are using APM in an AWS lambda function. APM Agents are designed to proxy their calls to the APM Server through the lambda extension. Instead, set `ELASTIC_APM_LAMBDA_APM_SERVER`. For more info, see <DocLink id="serverlessObservabilityApmAgentsPythonGetStarted">AWS Lambda</DocLink>.
...
<DocLink id="serverlessObservabilityApmAgentsPythonConfigure" section="dynamic-configuration"><DocBadge>Dynamic</DocBadge> </DocLink>
...
<div id="logging-options"></div>

## Logging Options

<div id="config-log_level"></div>
...
will do this for you) or set <DocLink id="serverlessObservabilityApmAgentsPythonConfigure" section="log_file">`log_file`</DocLink>. This will also override
any log level your app has set for the `elasticapm` logger.

<div id="config-log_file"></div>
...
at the `logging.logLevel` configured with <DocLink id="serverlessObservabilityApmAgentsPythonConfigure" section="log_level">`log_level`</DocLink>. Use
<DocLink id="serverlessObservabilityApmAgentsPythonConfigure" section="log_file_size">`log_file_size`</DocLink> to configure the maximum size of the log file. This log
file will automatically rotate.

Note that setting <DocLink id="serverlessObservabilityApmAgentsPythonConfigure" section="log_level">`log_level`</DocLink> is required for this setting to do
anything.

If [`ecs_logging`](https://github.com/elastic/ecs-logging-python) is installed,
the logs will automatically be formatted as ecs-compatible json.

<div id="config-log_file_size"></div>
...
The size of the log file if <DocLink id="serverlessObservabilityApmAgentsPythonConfigure" section="log_file">`log_file`</DocLink> is set.
...
<div id="config-log_ecs_reformatting"></div>
...
as `"off"`. See the
[`ecs_logging` docs](https://www.elastic.co/guide/en/ecs-logging/python/current/installation.html)
...
We recommend [Filebeat](https://www.elastic.co/beats/filebeat) for that purpose.

<div id="other-options"></div>

## Other options

<div id="config-transport-class"></div>
...
<div id="config-transactions-ignore-patterns"></div>
...
<DocCallOut title="Note">
as the the name of the transaction can only be determined at the end of the transaction,
the agent might still cause overhead for transactions ignored through this setting.
If agent overhead is a concern, we recommend <DocLink id="serverlessObservabilityApmAgentsPythonConfigure" section="transaction_ignore_urls">`transaction_ignore_urls`</DocLink> instead.
</DocCallOut>
...
The setting has to be provided in **<DocLink id="serverlessObservabilityApmAgentsPythonConfigure" section="duration-format">duration format</DocLink>**.
If a request to the APM Server takes longer than the configured timeout,
the request is cancelled and the event (exception or transaction) is discarded.
Set to `None` to disable timeouts.

<DocCallOut title="Warning" color="warning">
If timeouts are disabled or set to a high value,
...
<div id="config-auto-log-stacks"></div>
...
<div id="config-collect-local-variables"></div>
...
<DocCallOut title="Note">
Collecting local variables has a non-trivial overhead.
...
<div id="config-local-var-max-length"></div>
...
<div id="config-local-list-var-max-length"></div>
...
<div id="config-local-dict-var-max-length"></div>
...
<DocCallOut title="Warning" color="warning">
Request headers often contain sensitive values like session IDs and cookies.
See <DocBadge><DocIcon size="s" type="unlink" title="missing link"/> missing link</DocBadge>{/*  <DocLink id="enApmAgentPythonSanitizingData">sanitizing data</DocLink> */} for more information on how to filter out sensitive data.
</DocCallOut>
...
this setting should be provided in **<DocLink id="serverlessObservabilityApmAgentsPythonConfigure" section="duration-format">duration format</DocLink>**.

<div id="config-span-frames-min-duration"></div>
...
<DocCallOut title="Note">
This config value is being deprecated. Use
<DocLink id="serverlessObservabilityApmAgentsPythonConfigure" section="span_stack_trace_min_duration">`span_stack_trace_min_duration`</DocLink> instead.
</DocCallOut>
...
<div id="config-span-compression-exact-match-max_duration"></div>
...
<DocCallOut title="Note">
if a span propagates distributed tracing IDs, it will not be ignored, even if it is shorter than the configured threshold.
...
It has to be provided in **<DocLink id="serverlessObservabilityApmAgentsPythonConfigure" section="size-format">size format</DocLink>**.

<DocCallOut title="Note">
Due to internal buffering of gzip, the actual request size can be a few kilobytes larger than the given limit.
...
This setting is useful to limit memory consumption if you experience a sudden spike of traffic.
It has to be provided in **<DocLink id="serverlessObservabilityApmAgentsPythonConfigure" section="duration-format">duration format</DocLink>**.

<DocCallOut title="Note">
The actual time will vary between 90-110% of the given value,
to avoid stampedes of instances that start at the same time.
</DocCallOut>

<div id="config-processors"></div>
...
For more information, see <DocBadge><DocIcon size="s" type="unlink" title="missing link"/> missing link</DocBadge>{/*  <DocLink id="enApmAgentPythonSanitizingData">Sanitizing Data</DocLink> */}.
...
<DocCallOut title="Warning" color="warning">
We recommend always including the default set of field name matches
...
<div id="config-include-paths"></div>
...
(see [`fnmatch`](https://docs.python.org/3/library/fnmatch.html) for a description of the syntax).
...
<div id="config-exclude-paths"></div>
...
This setting is ignored when <DocLink id="serverlessObservabilityApmAgentsPythonConfigure" section="server_cert">`server_cert`</DocLink> is set.
...
<DocCallOut title="Note">
If this option is set, the agent only verifies that the certificate provided by the APM Server is
...
and provided by the [`certifi`](https://pypi.org/project/certifi/) package.
...
By default, the Python Agent uses the [`certifi`](https://pypi.org/project/certifi/) certificate store.
To use Python's default mechanism for finding certificates, set this option to `False`.

<div id="config-metrics_interval"></div>
...
<div id="config-breakdown_metrics"></div>
...
<div id="config-prometheus_metrics"></div>
...
See <DocBadge><DocIcon size="s" type="unlink" title="missing link"/> missing link</DocBadge>{/*  <DocLink id="enApmAgentPythonMetrics" section="prometheus-metric-set-beta">Prometheus metric set (beta)</DocLink> */} for more information.

<DocCallOut title="Note">
This feature is currently in beta status.
</DocCallOut>

<div id="config-prometheus_metrics_prefix"></div>
...
<div id="config-metrics_sets"></div>
...
See <DocBadge><DocIcon size="s" type="unlink" title="missing link"/> missing link</DocBadge>{/*  <DocLink id="enApmAgentPythonMetrics" section="custom-metrics">Custom Metrics</DocLink> */} for more information.

<div id="config-central_config"></div>
...
See <DocLink id="serverlessObservabilityApmAgentsPythonConfigure" section="dynamic-configuration">Dynamic configuration</DocLink> for more information.

<DocCallOut title="Note">
This feature requires APM Server and Kibana >= 7.3.
</DocCallOut>

<div id="config-global_labels"></div>
...
<div id="config-generic-disable-log-record-factory"></div>
...
By default in python 3, the agent installs a <DocBadge><DocIcon size="s" type="unlink" title="missing link"/> missing link</DocBadge>{/*  <DocLink id="enApmAgentPythonLogs" section="logging">LogRecord factory</DocLink> */} that
...
To enable [distributed tracing](((apm-guide-ref))/apm-distributed-tracing.html),
the agent sets a number of HTTP headers to outgoing requests made with <DocLink id="serverlessObservabilityApmAgentsPythonSupportedTechnologies" section="external-http-requests">instrumented HTTP libraries</DocLink>.
These headers (`traceparent` and `tracestate`) are defined in the [W3C Trace Context](https://www.w3.org/TR/trace-context-1/) specification.
...
- `'restart'`: Always ignores the `traceparent` header of incoming requests.
  A new trace-id will be generated and the sampling decision will be made based on <DocLink id="serverlessObservabilityApmAgentsPythonConfigure" section="transaction_sample_rate">`transaction_sample_rate`</DocLink>.
...
<div id="config-use-elastic-excepthook"></div>
...
<div id="config-include-process-args"></div>
...
<div id="config-django-specific"></div>

## Django-specific configuration

<div id="config-django-transaction-name-from-route"></div>
...
<div id="config-django-autoinsert-middleware"></div>
...
<div id="config-generic-no-proxy"></div>
...
<div id="config-ssl-cert-file"></div>
...
See also [OpenSSL docs](https://www.openssl.org/docs/manmaster/man7/openssl-env.html#SSL_CERT_DIR-SSL_CERT_FILE).
...
<DocCallOut title="Note">
These environment variables only take effect if <DocLink id="serverlessObservabilityApmAgentsPythonConfigure" section="use_certifi">`use_certifi`</DocLink> is set to `False`.
</DocCallOut>
