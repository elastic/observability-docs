...
For <DocBadge><DocIcon size="s" type="unlink" title="missing link"/> missing link</DocBadge>{/*  <DocLink id="enApmGuideApmRum">RUM</DocLink> */} send an `HTTP POST` request to the APM Server `intake/v2/rum/events` endpoint instead:
