...
You do not need to configure any settings to use APM; it's ready out-of-the-box with sensible defaults.
Depending on your use case however, you may find you want to fine-tune certain aspects of the UI.
...
: <DocBadge template="beta" /> When continuous rollups is enabled, the UI will select metrics with the appropriate resolution.
...
: Determines whether the comparison feature is enabled or disabled by default.
