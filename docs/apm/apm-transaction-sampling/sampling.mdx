...
See <DocLink id="serverlessObservabilityApmTransactionSampling">Configure head-based sampling</DocLink> to get started.
...
Because slower traces are more interesting than faster ones, tail-based sampling uses weighted random sampling &mdash; so
...
so non-sampled RUM traces retain transaction data &mdash; only span data is dropped.
