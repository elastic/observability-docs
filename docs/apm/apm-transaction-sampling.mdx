...
requires the sample rate to be propagated between services and the managed intake service.
Most samplers in OpenTelemetry do not support propagating sample rate,
which results in inaccurate span based metrics, e.g. APM throughput, latency, and error metrics.

[Consistent probability samplers](https://opentelemetry.io/docs/specs/otel/trace/tracestate-probability-sampling/)
in OpenTelemetry propagate the sample rate,
therefore span based metric extrapolation will work and you will see accurate span based metrics.
With other samplers, that don't propagate sample rate, span based metrics will be inaccurate.
...
Some visualizations in the Applications UI, like latency, are powered by aggregated transaction and span <DocLink id="serverlessObservabilityApmDataTypes">metrics</DocLink>.
