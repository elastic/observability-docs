...
Elastic does not have an internal queue to buffer requests,
but instead leverages an HTTP request timeout to act as back-pressure.
If Elastic goes down, incoming requests will eventually be denied.
Logs will be issued accordingly.

Some APM agents have internal queues or buffers that will temporarily store data if Elastic goes down.
As a general rule of thumb, queues fill up quickly. Assume data will be lost if Elastic goes down.
