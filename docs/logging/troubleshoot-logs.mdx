...
When adding a new data using the guided instructions in your project (**Add data -> Stream log files**),
...
* Get an API key from an administrator and manually add the API to the ((agent)) configuration. See <DocLink id="serverlessObservabilityStreamLogFiles" section="step-3-configure-the-elastic-agent">Configure the ((agent))</DocLink> for more on manually updating the configuration and adding the API key.
...
The host needs access to your project. If that's not possible, you can manually download and configure the ((agent)). See the <DocLink id="serverlessObservabilityStreamLogFiles">Stream a log file</DocLink> documentation for more information.
