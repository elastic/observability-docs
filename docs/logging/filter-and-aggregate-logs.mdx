...
Make sure the logs you're looking for fall within the time range in Log Explorer. If you don't see your logs, click the calendar icon <DocIcon type="calendar" title="calendar icon" /> to update the time range according to the timestamps in the example logs.
