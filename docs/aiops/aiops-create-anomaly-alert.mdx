...
1. Choose the result type:
    ...
    <DocTable columns={[{ title:  "Choose...", width: "25%" }, { title:  "To generate an alert based on...", width: "75%"  }]}>
        ...
        <DocRow>
            ...
            <DocCell>What individual anomalies that are present in a time range</DocCell>
        </DocRow>
        <DocRow>
...
The button is grayed out if the datafeed is not open.
...
For details about configuring a specific type of connector, refer to the [Connectors documentation](((kibana-ref))/action-types.htmlConnectors.html).
...
1. Save the rule.
1. If the datafeed is not currently running, go to **AIOps -> Anomaly detection** and start the job now.
...
1. On the **Job settings** tab, click the rule under **Alert rules** to edit the rule.


{/* TODO: QUESTION: Why doesn't the anomaly detection rule I created appear under Alerts > Manage Rules */}
{/* TODO: Figure out why rule doesn't seem to be working either */}

import Feedback from '../partials/feedback/widget.mdx'

<Feedback slug="/serverless/observability/aiops" filepath="docs/aiops/aiops-create-anomaly-alert.mdx" />
