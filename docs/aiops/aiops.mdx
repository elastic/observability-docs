---
id: serverlessObservabilityAiops
slug: /serverless/observability/aiops
title: AIOps
description: Automate anomaly detection and accelerate root cause analysis with AIOps.
tags: [ 'serverless', 'observability', 'overview' ]
status: in testing
---

import InReview from '../partials/in-review-notice.mdx'

<InReview />

The AIOps capabilities available in Elastic ((observability)) enable you to consume and process large observability data sets at scale, reducing the time and effort required to detect, understand, investigate, and resolve incidents.
Built on predictive analytics and machine learning, our AIOps capabilities require no prior experience with machine learning;
DevOps engineers, SREs, and security analysts can get started right away using these key AIOps features with little or no advanced configuration:

<DocTable columns={[{ title: "Feature" }, { title: "Description" }]}>
    <DocRow>
        <DocCell><DocLink id="serverlessObservabilityAiopsDetectAnomalies" text="Anomaly detection"/></DocCell>
        <DocCell>Detect anomalies by comparing real-time and historical data from different sources to look for unusual, problematic patterns.</DocCell>
    </DocRow>
    <DocRow>
        <DocCell><DocLink id="serverlessObservabilityAiopsAnalyzeSpikes" text="Log rate analysis"/></DocCell>
        <DocCell>Find and investigate the causes of unusual spikes or drops in log rates.</DocCell>
    </DocRow>
    <DocRow>
        <DocCell><DocLink id="serverlessObservabilityAiopsDetectChangePoints" text="Change point detection"/></DocCell>
        <DocCell>Detect distribution changes, trend changes, and other statistically significant change points in a metric of your time series data.</DocCell>
    </DocRow>
</DocTable>

These features are designed to work with alerting, case management,
and other incident management features that let you tie into additional third-party systems and integrate with your team's workflows.

import Feedback from '../partials/feedback/widget.mdx'

<Feedback slug="/serverless/observability/aiops" filepath="docs/aiops/aiops.mdx" />
