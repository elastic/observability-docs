...
{/* QUESTION: Will the ML APIs work? Do we want to talk about them in the docs? */}
...
Each anomaly detection contains one or more _detectors_, which define the type of
...
To learn more about anomaly detection, refer to the [Machine Learning](((ml-docs))/ml-ad-overview.html) documentation.
Note that the Machine learning documentation may contain details that are not valid when using a fully-managed Elastic project.
...
You might also see specialized wizards based on the type of data you are analyzing. certain types of data and provide specialized wizards for that context.
...
If you want the job to start immediately when the job is created, make sure you select that option.
You can skip advanced settings for now and <DocLink id="serverlessObservabilityAiopsTuneAnomalyDetectionJob" text="tune the job" /> later.
...
<DocCallOut title="Tip">
    Expected values are available only if the job has **Enable model plot** selected under Job Details.

</DocCallOut>

To explore your data:

1. If the **Single Metric Explorer** is not already open, go to **AIOps -> Anomaly detection** and click the appropriate icon next to the job you created.
The Single Metric Explorer icon is grayed out for advanced or multi-metric jobs.
...
You can customize the actions menu by defining custom URLs.
To learn more about job rules and defining custom URLs, refer to <DocLink id="serverlessObservabilityAiopsTuneAnomalyDetectionJob" />
...
To learn more about influencers, refer to the [Machine Learning](((ml-docs))/ml-ad-influencers.html) documentation.
Note that the Machine learning documentation may contain details that are not valid when using a fully-managed Elastic project.
...
1. If the **Anomaly Explorer** is not already open, go to **AIOps -> Anomaly detection** and click the appropriate icon next to the job you created.
1. In the time filter, specify a time range that covers the majority of the analyzed data points.
...
1. Under **Anomaly timeline**, click a section in the swim lanes to obtain more information about the anomalies in that time period.
    {/* TODO: Need to add screen capture and example here...maybe the example from the tutorial? */}
    ...
    You can also switch to viewing this time series in the **Single Metric Viewer** by clicking the **View Series** button in the **Actions** menu.
1.  Under **Anomalies** (in the **Anomaly Explorer**), expand each anomaly to see key details, such as the time,
the actual and expected ("typical") values, and the influencers that contributed to the anomaly.
For example:

    ![Anomaly Explorer showing anomaly details ](../images/anomaly-detection-details.png)
    {/* TODO: Probably need a unique screen capture...maybe the example from the tutorial? */}
    ...
    {/* TODO: Need to add a realistic example, maybe from the tutorial? */}
...
* <DocLink id="serverlessObservabilityAiopsDetectAnomaliesAlerts" text="Create an anomaly detection alert" />
