...
<DocCallOut title="Tip">
    You can also create calendars and add URLs when configuring advanced settings during job creation,
...
The machine learning model is not ill-affected, and you do not receive spurious results.
...
1. Under **Events**, click **New event** or click **Import events** to import events from an iCalendar (ICS) file:
    ...
    A scheduled event must have a start time, end time, and description.
...
The scope is defined by using machine learning filters.
</DocCallOut>


*Filters* contain a list of values that you can use to include or exclude events from the machine learning analysis.
...
This gives you much greater control over which anomalous events affect the machine learning model and appear in the machine learning results.
</DocCallOut>
...
{/* TODO: Add screen capture showing job rules definition populated with realistic settings. */}
...
{/* TODO: Create tracking issue to add a topic called "Manage machine learning jobs that describes how to:

Manage model snapshots: https://www.elastic.co/guide/en/machine-learning/current/ml-ad-run-jobs.html#ml-ad-model-snapshots

Open and close jobs?

Stop and start datafeeds?

Handle delayed data? */}

import Feedback from '../partials/feedback/widget.mdx'

<Feedback slug="/serverless/observability/aiops" filepath="docs/aiops/aiops-tune-anomaly-detection-job.mdx" />
