...
Mappings are defined in the `fields` directory and are used to create component templates for the integration. Note that the names of these files are conventions, deviations are allowed.
...
=== base-fields.yml
The fields defined in this file are placeholders for the datastream subfields which, in turn, receive their value from the Elastic agent.
...
For example, in the Apache integration (more specifically, for the access dataset) that would be:
* event.module: apache
...
The `external: ecs` definition instructs the `elastic-package` command line tool to refer to an external ECS reference to resolve specific fields. By default it looks at the https://raw.githubusercontent.com/elastic/ecs/v8.6.0/generated/ecs/ecs_nested.yml[ECS reference] file hosted on Github. This external reference file is determined by a Git reference found in the `_dev/build/build.yml` file, located starting from the root of the package directory.
