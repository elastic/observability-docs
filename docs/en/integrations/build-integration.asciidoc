...
Mappings are defined in the `fields` directory and are used to create component templates for the integration.
...
=== ecs.yml
This file specifies every ECS field used by the integration that is not defined in the files `agent.yml` or `base-fields.yml` files.
...
The `external: ecs` definition instructs the `elastic-package` binary to refer to an external ECS reference to resolve these fields. By default it looks at the ECS https://raw.githubusercontent.com/elastic/ecs/v8.5.1/generated/ecs/ecs_nested.yml[reference] file on Github, following the Git reference in the `_dev/build/build.yml` file located at the root of the package directory.
By changing the reference to the ECS reference file in `_dev/build/build.yml` from `Git@..` to `file:///file/path/ecs_nested.yml`, it is possible to look for this file locally.
// Can we also use "external" references in the other files like agent.yml?
