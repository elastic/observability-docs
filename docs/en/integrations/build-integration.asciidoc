...
The <<elastic-package,`elastic-package`>> tool provides a quick way to spin up the Elastic Stack.
The following command deployes {es}, {kib}, and the Package registry:
...
For a complete list of options available for this command, run:
...
When complete, navigate to http://localhost:5601 and login with the username, `elastic` and password, `changeme`.
...
We recommend using the `elastic-package create` command to build a new package,
rather than copying the source of an existing package. This ensures your integration is following the latest recommendations for the package format.
...
The wizard will walk you through the creation of the package, including setting a package name, version, category, etc.
...
[NOTE]
====
It may not do anything yet, but from this step onwards,
your integration can be built and loaded into your locally running package registry.
...
A data stream is logical sub-division of an integration package,
dealing with a specific type of observable aspect of the service or product being observed. For example,
...
These assets are loaded into {es} when a user installs an integration via the Fleet UI in Kibana.

A data stream also defines a policy template.
Policy templates include variables that allow users to configure the data stream via the Fleet UI in {kib}.
The resulting policy is interpreted by the {agent} to collect relevant information from the product or service being observed.
...
Run this command each time you add a new data stream to your integration.
...
In most instances, data needs to be manipulated before it is ingested into the Elastic Stack -- for example,
...
Descriptions are shown in the Kibana UI, so try and keep them clean and consistent.
...
. Verify that your integration is available (in the right version), e.g. MySQL: http://localhost:8080/search?package=mysql (use `experimental=true` parameter if the package is in experimental version.
...
. (Optional) Download the Elastic-Agent from https://www.elastic.co/downloads/elastic-agent
