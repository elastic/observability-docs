...
The _dev directory is part of the link:https://github.com/elastic/package-spec[package-spec], containing development resources. These development resources cover any types of files/folders needed only at development time. This includes resources needed for testing but also includes any templates that might be used for generating documentation. In the future it could include other files/folders needed just at development time. It can be defined on the following levels:

. The package-level `_dev` folder contains files needed to setup the testing environment for that package. This environment setup is specified via folders/files in the `_dev/deploy` folder. For example, the `apache` package link:https://github.com/elastic/integrations/tree/main/packages/apache/_dev/deploy[specifies] how to spin up an Apache Docker container for testing.
