...
In this tutorial, we are going to assume that your logs and your infrastructure
...
Filebeat, the agent that we are going to use to collect logs, has an input for
S3.
...
also need an SQS queue. In order to avoid significant lagging with polling all
log files from each S3 bucket, we will use Amazon Simple Queue Service (SQS).
This will provide us with Amazon S3 notification when a new S3 object is
...
and reliability. So what we need to do at this point is to create a SQS queue
...
created and then to *Event notification*.

image::configure-event-notification.png[Event Notification Setting]

Specify that you want to send notification every time an object is being created
and that you will send the notification to the SQS queue you just created.
...
[discrete]
==== Step 5: Configuring the {aws} Module

Now that the output is working, you are going to set up the {aws} module which
is going to automatically going to define the {aws} input. You are going to use
the {filebeat-ref}/filebeat-module-aws.html[Amazon Web Services] module. This
...
There are many diferent filesets available: `cloudtrail`, `vpcflow`, `ec2`,
`cloudwatch`, `elb` and `s3access`. In this tutorial we are going to show you a
few examples using the `ec2` and the `s3access` filesets. The `ec2` fileset is
used to ship and process logs stored in cloudwatch and exported them to a S3
...
make sure to edit the policy of your bucket like shown in the https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/S3ExportTasksConsole.html[step 3 of the {aws} user guide]
it will help you avoiding permissions issues.
...
Let's now configure another fileset and we are going to use the `s3access`
one. The goal here is to be able to monitor how people access the bucket we
created. This is what we achieved:

image::one-bucket-archi.png[Current Architecture]

Now what we need to do is to create another bucket and another queue. In order
to create the following architecture:

image::two-buckets-archi.png[Architecture with Access Logging Enabled]

Make sure the event notifications on the bucket you created is enabled and is
sending notifications to the new queue you also just created. Now you need to go
back to the first bucket you created, go to the properties, go to **Server access
logging**, specify that you want to ship the access logs to the bucket you last
created.
...
Similarly to {filebeat}, {metricbeat} comes with predefined assets for parsing,
...
[discrete]
==== Step 9: Configuring the {aws} module

To collect the metrics from your {aws} infrastructure we will use the
{metricbeat-ref}/metricbeat-module-aws.html[{aws}] module. The {aws} module is
containing a lot of metricsets: `billing`, `cloudwatch`, `dynamodb`, `ebs`,
`ec2`, `elb`, `lambda` and many more. Each metricsets is created to help you
stream and process your data. In this tutorial we are going to show you a
...
Kibana. Go to Kibana, in the interface click on the left panel, click on
**Metrics** to see how your infrastructure look like. Once in the **Metrics UI**
make sure to show the **{aws}** source and the **EC2 Instances**:
