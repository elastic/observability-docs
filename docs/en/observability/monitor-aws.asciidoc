...
Filebeat, the agent that we will use to collect logs, has an input for S3.
...
You have created an S3 bucket. This will be where you can export your logs.
This doesn't notify you when users create new S3 objects. Rather than poll
for all log files in each S3 bucket, use Amazon Simple Queue Service (SQS).
This provides you with an Amazon S3 notification when someone creates a new
S3 object. The {filebeat-ref}/filebeat-input-aws-s3.html[Filebeat S3 input]
checks SQS for messages about new S3 objects. It uses the information in
these messages to retrieve logs from S3 buckets. With this setup, you don't
need periodic polling from each S3 bucket. The Filebeat S3 input guarantees
reliable, near real-time data collection from S3 buckets.
