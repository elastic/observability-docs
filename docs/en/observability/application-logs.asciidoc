...
The ingestion of those logs requires to deal with two aspects:

- <<log-types,log format>>: plaintext or ECS (structured JSON) logs.
- <<ingest-application-logs,logs ingestion>>: how those logs are sent and written to Elasticsearch using a tool like {filebeat}.

The choice of the log format influences the log ingestion strategy.
For simplicity, this documentation will focus on {filebeat}, but any log ingestion tool could be used with equivalent configuration.
...
- <<log-correlation,log correlation>>: ability to correlate logs to APM traces
...
There are two ways to make the application write ECS formatted logs
- using {ecs-logging-ref}/intro.html[ECS logging] library
- using an APM agent and <<log-reformatting,log reformatting>>
...
Here {filebeat} is used for reference, but any equivalent ingestion tool
could be used with equivalent configuration.
