...
- log format: plaintext or ECS (structured JSON) logs (see <<log-type, below>> for details)
- logs ingestion: how those logs are sent and written to Elasticsearch using a tool like {filebeat}.

The choice of the log format influences the log ingestion strategy.
For simplicity, this documentation will focus on {filebeat}, but any log ingestion tool could be used
with equivalent configuration.

Using an APM agent provides extra features:
- log correlation: ability to correlate logs to APM traces
