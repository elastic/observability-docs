...
(<<synthetics-lightweight-data-string,string>>)
a| APM service name for this monitor. Corresponds to the `service.name` ECS field. Set this when monitoring an app that is also using APM to enable integrations between Synthetics and APM data in Kibana or your serverless project.
...
////////////////////////
| [[monitor-alert]] *`alert`*
status_badge::[cloud_hosted, added, 8.9.0] status_badge::[synthetics_cli, added, 1.3.0]
...
* Run the <<elastic-synthetics-locations-command,`elastic-synthetics locations` command>> and specify the URL of the deployment or serverless project. This will fetch all available private locations associated with the deployment.
