...
Click *Open history* to see the corresponding values of the metric you chose in 
the selected time frame. Detected anomalies with an anomaly score equal to or 
higher than 50 are highlighted on the chart with red. Use the 
{ml-docs}/ml-gs-results.html[Anomaly Explorer] to examine the detected 
anomalies.
