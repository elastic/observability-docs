...
= Overview
The Elastic Serverless Forwarder can forward {aws} data to cloud-hosted, self-managed Elastic environments or Logstash (Technology Preview). It supports the following inputs:
...
[role="screenshot"]
image::images/aws-serverless-lmabda-flow-logstash.png[AWS Lambda flow]
...
You can use the <<sample-s3-config-file,config.yaml>> file to configure the service for each input and output type, including information such as SQS queue ARN (Amazon Resource Number), {es} and {ls} connection details. You can create multiple input sections within the configuration file to map different inputs to specific log types.
