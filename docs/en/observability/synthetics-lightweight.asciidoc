...
// temporary. remove after #2593 is merged.
...
To use the UI in the {synthetics-app}, <<synthetics-get-started-ui,go to the {synthetics-app}>>
in {kib} to create and configure monitors.
...
To use YAML files in {project-monitors}, <<synthetics-get-started-project,Set up a project>>
and configure monitors in YAML files in the `journeys` directory.
...
The options described here configure Heartbeat to connect via HTTP and
...
and the following HTTP-specifc options:
...
// Is this still relevant?
The options described here configure Heartbeat to use ICMP (v4 and v6) Echo
...
`sudo setcap cap_net_raw+eip /path/to/heartbeat` to  grant Heartbeat ping capabilities on Linux.
...
and the following ICMP-specifc options:
...
// Is this still relevant?
The options described here configure Heartbeat to connect via TCP and
...
and the following TCP-specifc options:
