...
Lightweight monitors can be setup via either:

1. The GUI (beta)
2. YAML files in Project Monitors (beta)
3. YAML files in Heartbeat (GA)

Note that YAML files in Project Monitors and the GUI support the most common subset of options that Heartbeat does, but that work is ongoing to bring it closer to parity.
The examples here will use Heartbeat YAML for illustrative purposes, note that these options map back to similarly named GUI options.
...
monitoring tests via Synthetic Agent on the Chromium browser.
