...
. Create a resource group, event hub namespace, and event hub for your logs. To
learn how, refer to
...
. Make a note of the event hub name because you will need it later.

//REVIEWERS: Do we have any recommendations for naming conventions for the
//resource group, event name, etc?

****
**When do I need more than one event hub?**

Typically you create an event hub for each service you want to monitor. For
example, imagine that you want to collect active directory logs from the Azure
Monitor service and signin and audit logs from the Active Directory service.
...
// QUESTION: Are these steps correct? The current (not updated) integration docs
// send users to this link for steps, but TBH these docs are confusing:
// https://docs.microsoft.com/en-us/azure/azure-monitor/platform/activity-log-export

// I was able to follow these steps to get logs for my own user account, but
// not sure if I missed something along the way.

//TODO: Look more closely at Maurizio's updates to the integration docs.
...
. Log into the Azure portal and go to **Home -> Monitor**.
. In the navigation page, select **Activity log** then click
**Export Activity Logs**.
. Click **Add diagnostic setting** and enter a name for the diagnostic setting.
. In the list of log categories, select the logs you want to export. For this
tutorial, select ____.
. Under Destination details, select **Stream to an event hub** and select the
event hub namespace you created earlier.
. Save your changes.

//REVIEWERS: Which logs should we tell users to colllect ^^. 
...
+
[horizontal]
**Eventhub**:: The name of the main event hub.

**Connection String**:: The connection string primary key of the event hub. To
learn how to find the connection string, refer to
...
to store, retrieve, and update offset and state information. You can create a
new storage account, or use an existing one.
//REVIEWERS: Is this true? I'm tending towards not telling users how to create
//a blob storage account because I think Azure users would all ready know how
//to do that. Maybe I should put this as a prereq though?

**Storage account key**:: A valid access key defined for the storage account.

. Make sure the **Collect Azure activity logs** selector is turned on.

. Add the integration to the agent policy that contains your billing metrics
integration, then click **Save and continue**. This step takes a minute or two
to complete. When it's done, you'll have an agent policy that contains the Azure
activity logs configuration you just specified plus configuration that you added
earlier.
...
//REVIEWERS: The "failed to find message" messages are not a good look, but I
