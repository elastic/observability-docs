...
//REVIEWERS: As you can see, the dashboard is not displaying any forecast data.
//Does the service principal need to have the Billing Reader role? Just the
//Reader role? Or both?
...
//COMMENT: Since the setup for activity logs is very different from active
//directory logs, should we show both?
