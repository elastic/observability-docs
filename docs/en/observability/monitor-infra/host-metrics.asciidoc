...
//REVIEWERS: Note that the metrics are sorted alphabetically in the tables. This might
//not be ideal for some metrics that are closely related, but it's the best
//organizing principle for reference content because it's easier to scan and
//maintain over time. Other organizing principles fall apart over time as people
//make updates to add/remove content. If you think related metrics need to be
//discussed next to each other, I can break the section down into more categories.
