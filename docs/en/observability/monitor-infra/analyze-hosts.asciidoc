...
[NOTE]
=====
For alerts to appear on the Hosts page,
the rule that triggers the alert must associate the host name with the alerts it creates.
The way you do this depends on the rule type:
