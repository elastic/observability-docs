...
before a host name with a note in a tooltip stating that the host has been detected by APM.
// Why it happens
When a host is detected by APM, but is not collecting full metrics
(for example, via the https://www.elastic.co/docs/current/integrations/system[system integration]),
