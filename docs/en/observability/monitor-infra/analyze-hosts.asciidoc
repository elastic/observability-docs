...
[discrete]
[[analyze-hosts-troubleshooting-unexpected-host-names]]
=== Incorrect host names and metrics set to N/A

// What the user sees/experiences (error message, UI, behavior, etc)
If you see unexpected host names in the Hosts view, metrics for that host are set to `N/A`,
_and_ the host was detected by APM (signified by a question mark icon before the host name),
// Why it happens
it's likely that the APM agent has not been configured to use the correct host name.
