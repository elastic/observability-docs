...
`app.before()` handler and wrapping a logging handler to all handlers.
+
[source,java]
----
app.get("/", log(mainHandler()));

// wrap all other handlers within log() as well
// left as an exercise for the reader

public static Handler log(final Handler handler) {
    return ctx -> {
