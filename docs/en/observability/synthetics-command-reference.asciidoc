...
*`--screenshots`*::
Enables capturing screenshots for every step in the journey.

*`--network`*::
Capture network requests for every step in a the journey.

*`-h, --help`*::
Shows help for the `npx @elastic/synthetics` command.s
