...
To set up an uptime monitor, you need to configure the monitor and then run it and send data back to Elasticsearch.
Elastic provides Docker images to run monitors to reduce the chance of problems.
...
Also, if you want to heavily script the creation and destruction of monitors, it can be less convenient than using <<uptime-set-up-choose-heartbeat>> directly.
...
* *Create a separate {fleet} policy to use for Synthetics monitors*: It's generally the case that synthetics monitors should run on nodes dedicated to that purpose.
