...
For example, set up global state or a server that will be used for a single `journey` using `before` or across more than one journey using `beforeAll`.
...
Or you can remove a global state or close a server used for a single `journey` using `after` or across more than one journey using `afterAll`.
