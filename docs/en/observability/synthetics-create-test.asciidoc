...
This {playwright-api-docs}[Playwright page API reference] is helpful while writing tests.
...
* Run as an inline journey -- copy and paste into `heartbeat.yml`
...
In this method, use Docker to run both Heartbeat and `elastic-synthetics`.
...
There's an example provided in the
