...
// Does it make sense to include this here or should it be included in
// Run synthetic tests > Use a test suite > Send to {stack}
// since we don't know what a push command is yet?

[ Configure monitor-related fields using the Monitor DSL. ]
