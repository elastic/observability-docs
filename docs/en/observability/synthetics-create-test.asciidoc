...
Note that you can only paste the steps into an inline script, so these cannot be dependent on any components that you may want to `import`. If you do have any dependencies, these would need to be defined with an `import` outside of the `journey` object and run via a test suite.
