...
`beforeAll`::   Runs a provided function before all `journey` runs.
                If the provided function is a promise, the runner will wait for the
                promise to resolve before invoking the `journey`.
                Takes one parameters: a `callback` (function).
                See <<before-after>>.
`before`::      Runs a provided function before a single `journey` runs.
                Takes one parameters: a `callback` (function).
                See <<before-after>>.
`afterAll`::    Runs a provided function after all the `journey` runs have completed.
                Takes one parameters: a `callback` (function).
                See <<before-after>>.
...
It's reliable and fast and features a modern API that auto waits for page elements to be ready.
...
Set up global state or a server that will be used for a single `journey` using `before` or across one or more journeys using `beforeAll`.
...
Remove a global state or close a server used for a single `journey` using `after` or across one or more journeys using `afterAll`.
