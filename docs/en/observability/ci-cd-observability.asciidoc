...
The Jenkins OpenTelemetry Plugin also provides pipeline log storage in {es} while enabling you to
either visualize the logs in {kib} or continue to display them through the Jenkins pipeline build console.
...
The Jenkins OpenTelemetry Plugin provides turnkey pipeline log storage in {es} with visualization in {kib}.
The Jenkins pipeline build console will display a hyperlink to {kib} rather than displaying the logs on the page.
