...
Jenkins pipeline build logs can be sent through OpenTelemetry Protocol (OTLP) to be stored
...
due to the unification of all the signals in the observability backend.
...
Jenkins and limiting the well known file system performance of Jenkins when storing a large history of builds.
...
* The OpenTelemetry Protocol endpoint configured on the Jenkins OpenTelemetry integrationmust be
...
* When using OpenTelemetry Collectors, set up a logs pipeline in addition to the traces and metrics pipelines.
...
[role="screenshot"]
image::images/ci-cd-visualize-logs-kibana-console.png[Jenkins Console Output page with link to view logs in Elastic Observability]

.Architecture for storing pipeline logs in Elastic and visualizing logs exclusively in Elastic
image::images/ci-cd-visualize-logs-kibana-architecture.png[Architecture diagram for storing pipeline logs in Elastic and visualizing logs exclusively in Elastic]

[discrete]
[[ci-cd-visualize-logs-kibana-and-jenkins]]
=== Visualize in both Kibana and Jenkins
...
[role="screenshot"]
image::images/ci-cd-visualize-logs-kibana-and-jenkins-console.png[Jenkins Console Output page displaying both log contents and a link to view logs in Elastic Observability]
...
image::images/ci-cd-visualize-logs-kibana-and-jenkins-architecture.png[Architecture diagram for storing pipeline logs in Elastic and visualizing logs in both Elastic and Jenkins]
