...
To configure GitHub to send traces, refer to the https://github.com/elastic/otel-export-trace-action[tracing] docs.
In the GitHub configuration, you just need to define `OTEL_HEADERS`
and `OTLP_ENDPOINT` secrets. Refer to https://docs.github.com/en/actions/security-guides/encrypted-secrets[encrypted-secrets] docs.

[source,bash]
----
OTLP_ENDPOINT=elastic-apm-server.example.com:8200
OTLP_HEADERS=Authorization=Bearer your-secret-token
----

You can now create a GitHub workflow to listen for `workflow` runs that are completed:
...
Once GitHub tracing is configured, GitHub actions are
reported in Elastic {observability}.

[role="screenshot"]
.A GitHub action execution in Elastic {observability}
image::images/cicd-github.png[GitHub action execution]

You can additionally, send traces for the test results, so far JUnit is the only
supported test runner. In order to do so, you can configure your GitHub workflows
that produce any JUnit output to call https://github.com/elastic/otel-upload-test-artifact-action,
this will automatically propagate the context.
...
[role="screenshot"]
.A GitHub action execution with test report in Elastic {observability}
image::images/cicd-github-with-junit.png[GitHub action execution with Tests]
