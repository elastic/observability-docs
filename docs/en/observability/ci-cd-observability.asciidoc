...
Integrating automated service health checks in deployment pipelines is critical to end-to-end deployment automation which is critical to increase the deployment frequency .
...
The URL is `KIBANA_URL//internal/apm/services`, it is desing to query some service metrics.
In order to check the transaction error rate, we have to pass some parameters to the API call.
We need a rande of time, this range is defined by the parameters `start` and `end`, 
these parameters are two dates in ISO-8601 format. Decause we want to query only one service,
we have to compose a filter in the param `kuery`, we can filter by the service name using the expresion `service.name:MY_SERVICE_NAME`.
...
[source,python]
----
def check_service_health(service_name, error_rate_threshold, kibana_url, api_token):
    now = datetime.now()
    five_minutes_ago = now - timedelta(minutes=5)
    params = {
        "start": five_minutes_ago.strftime("%Y-%m-%dT%H:%M:%SZ"),
        "end": now.strftime("%Y-%m-%dT%H:%M:%SZ"),
        "kuery": "", # "service.name:{}".format(service_name),
    ...
    with urllib.request.urlopen(req) as response:
     ...
     if len(obj['items']) > 0 and obj['items'][0].transactionErrorRate > error_rate_threshold:
            raise Exception("Error rate for service {} is higer than threshold {}, current value is {}".format(service_name, error_rate_threshold, obj['items'][0].transactionErrorRate))
----
