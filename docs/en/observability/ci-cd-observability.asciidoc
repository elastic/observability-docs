...
Integrating automatedservice health checks in deployment pipelines is critical to end to end deployment automation.

Elastic Observability exposes service HTTP health check APIs that are intended to be invoked by deployment pipelines.

/internal/apm/services/detailed_statistics?



[source,python]
----
func check_service_health(service_name, error_rate_threshold, kibana_url, api_token)
  now = datetime.now()
  ...
  params = {
    ...
    "serviceNames": "[\"{}\"]".format(service_name),
    "offset": "1d"
  }
  url = "{}/internal/apm/services/detailed_statistics?{}".format(kibana_url, urllib.parse.urlencode(params))
  req = urllib.request.Request(url=url, headers={"Authorization": "Bearer {}".format(api_token)})
  with urllib.request.urlopen(req) as response:
     body = response.read().decode("utf8")
     obj = json.loads(body)
     if obj["currentPeriod"]:
       if obj["currentPeriod"][service_name]:
         if obj["currentPeriod"][service_name]["transactionErrorRate"]:
           for item in obj["currentPeriod"][service_name]["transactionErrorRate"]:
             error_rate = getattr(item, 'y', None)
             if error_rate > error_rate_threshold or error_rate == None:
               raise Exception("Error rate for service {} is higer than threshold {}, current value is {}".format(service_name, error_rate_threshold, error_rate))
