...
Elastic Observability exposes service HTTP health check APIs that can be integrated in deployment pipelines to verify the behaviour of newly deployed instances, and either automatically continue the deployments or rollback according to the health status.
...
To check the transaction error rate of the service on the newly deployed instance, pass the following parameters to the API call:
