...
The Ansible OpenTelemetry plugin integration supports Ansible AWX/Tower, if you'd like to enable it please follow the below steps.

AWX requires an Execution Environment with the Ansible and Python packages installed.
For this we used the https://www.ansible.com/blog/introduction-to-ansible-builder tool to create the container definition.

You then upload the container to an image repository accessible by AWX and define an Execution Environment using the container you created.

To inject the environment variables and service details you can use custom credential types and then assign the credentials to the Playbook template.
...
Finally, https://www.elastic.co/blog/5-questions-about-ansible-that-elastic-observability-can-answer provides a great
overview of how all those pieces work together.
