...
This map can be changed to a table view.

Without leaving the *Inventory* page, view enhanced metrics relating to each host
...
To help you get started with your analysis faster, change the high-level view from
...
Sort by resource, group the resource by specific fields related to it, and sort by
either name or metric value. For example, filter the view to display the memory usage
...
AFter you've searched and filtered for a specific resource, drill down further to analyze the
