...
Now that the metrics are streaming to {es}, you can visualize them in {kib}.
Find **Dashboards** in the main menu or use the {kibana-ref}/introduction.html#kibana-navigation-search[global search field].
Search for Azure Billing and select the dashboard called **[Azure Billing] Billing Overview**.
...
Now that logs are streaming into {es}, you can visualize them in {kib}.
To see the raw logs, find **Discover** in the main menu or use the {kibana-ref}/introduction.html#kibana-navigation-search[global search field]. Notice that you
can filter on a specific data stream. For example, you can use
