...
To see the logs from the VM, open **Logs Explorer** (search for `logs explorer` in the {kibana-ref}/introduction.html#kibana-navigation-search[global search field]).
+
To see the VM metrics, find **Infrastructure** in the main menu or use the {kibana-ref}/introduction.html#kibana-navigation-search[global search field].
