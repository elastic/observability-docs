...
per the field of your choice. Group and create visualizations of metrics
for one or more resources that you are monitoring.

Additionally, for a detailed analysis of your metrics, you can annotate and save visualizations for
...
+
2. In the *graph per* drop-down, add `host.name`.
+
...
value, so from the aggregation drop-down, select `Rate`.
+
...
+
8. To calculate the network traffic for all the interfaces, from the *group by* drop-down, select `Terms` and add the 
...
select `Series Agg`, and from the *Function* drop-down, select `Sum`.
+
