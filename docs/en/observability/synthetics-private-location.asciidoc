...
Any private location can only run two browser tests simultaneously, and as many lightweight checks as resources allow.
...
A good rule of thumb is to allocate at 2 GiB of memory and 2 cores per browser instance in order to have more consistent
performance and avoid out of memory errors. Resource requirements will vary depending on workload. 
For lightweight monitors much less memory is needed. We recommend starting at 512MiB of memory and two cores for 
lightweight checks and increasing allocated memory and CPU based on observed usage patterns.

Note that these limits are for simultaneous tests, not total tests. If, for instance,
60 browser tests were scheduled to run once per hour, with each taking exactly 1 minute to run that would fully occupy one execution slot.
That being said, it is not a good practice to leave execution slots without extra capacity. A good starting point would be to over-allocate by
a factor of 5. In the previous example that would mean mean allocating 5 slots.