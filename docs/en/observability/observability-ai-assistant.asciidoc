...
(*TBD* Add screenshot of how the UI looks?)
...
[TIP]
====
{enterprise-search-ref}/connectors.html[Search Connectors] differ from the {kib-ref}/action-types.html[Stack Connectors] configured during the <<obs-ai-set-up, AI Assistant setup>>.
...
Click *AI Assistant* icon (*TBD*) in the upper-right corner of any {observability} application to start the chat:
