...
The Observability AI Assistant is a large language model (LLM) integration that helps Elastic Observability users by adding context, explaining errors and messages, and suggesting remediation in the {observability} interface. A connector for OpenAI or Azure OpenAI Service powers the Observability AI Assistant. 
...
* An account with a third-party generative AI provider, which the Observability AI Assistant uses to generate responses. Supported providers are OpenAI (`gpt-4` model) and Azure OpenAI Service (`gpt-4` model).
...
. Create an API key with your AI provider to authenticate requests from the Security Assistant. You'll use this in the next step. Refer to the provider's documentation for generating API keys:
+
...
+
.. Add one of the following configurations that corresponds the AI service you're using:
+
** *OpenAI*:
+
...
#
xpack.observability.aiAssistant.provider.openAI.apiKey: xxxx
# 
xpack.observability.aiAssistant.provider.openAI.model: gpt-4
----
+
...
xpack.observability.aiAssistant.enabled: true
#
xpack.observability.aiAssistant.provider.azureOpenAI.deploymentId: gpt-4
# 
xpack.observability.aiAssistant.provider.azureOpenAI.resourceName: xxxx
# 
xpack.observability.aiAssistant.provider.azureOpenAI.apiKey: xxxx
----
...
- *Universal Profiling* – explains the most expensive processes and functions in your fleet and provides optimization suggestions.
...
For example, in the logs details, you'll see prompts for *What's this message?* and *How do I find similar log messages*: 
