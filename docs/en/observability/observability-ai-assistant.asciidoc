...
* An {enterprise-search-ref}/server.html[Enterprise Search] server if Search Connectors are used for the knowledge base
...
AI Assistant Settings are available through Kibana Stack Management and directly from the AI Assistant clicking on the three dots.
...
You can also add external data to the knowledge base in the following ways:
* Using the Stack Management UI in {kib}.
