...
To add data to the AI Assistant knowledge base index:

. Ingest external data (Github Issues, Markdown files, Jira tickets, text files, etc.) into {es} one of the following ways:
...
. Reindex your data into the AI Assistant's knowledge base index using the following query in *Management* -> *Dev Tools* in {kib}. Update the following fields before reindexing:
** `InternalDocsIndex` — name of the index where your internal documents are stored.
** `text_field` — name of the field with the text of your internal documents.
** `timestamp` — name of the field of the timestamp in your internal documents.
