    ...
    Resources:
      SarCloudformationDeployment:
        ...
        Properties:
          ...
          Parameters:
            ElasticServerlessForwarderS3ConfigFile: ""          ## ENTER THE VALUE OF THE S3 URL IN THE FORMAT "s3://bucket-name/config-file-name" POINTING TO THE CONFIGURATION FILE FOR YOUR DEPLOYMENT OF THE ELASTIC SERVERLESS FORWARDER
            ...
            ElasticServerlessForwarderSQSEvents: ""             ## ENTER A COMMA DELIMITED LIST OF DIRECT SQS QUEUES ARNS TO SET AS EVENT TRIGGERS FOR THE LAMBDA (IF ANY).
            ElasticServerlessForwarderS3SQSEvents: ""           ## ENTER A COMMA DELIMITED LIST OF S3 SQS EVENT NOTIFICATIONS ARNS TO SET AS EVENT TRIGGERS FOR THE LAMBDA REFERENCED IN THE CONFIG YAML FILE (IF ANY).
            ElasticServerlessForwarderKinesisEvents: ""         ## ENTER A COMMA DELIMITED LIST OF KINESIS DATA STREAM ARNS TO SET AS EVENT TRIGGERS FOR THE LAMBDA REFERENCED IN THE CONFIG YAML FILE (IF ANY).
            ElasticServerlessForwarderCloudWatchLogsEvents: ""  ## ENTER A COMMA DELIMITED LIST OF CLOUDWATCH LOGS LOG GROUPS ARNS TO SET SUBSCRIPTION FILTERS ON THE LAMBDA REFERENCED IN THE CONFIG YAML FILE (IF ANY).
            ElasticServerlessForwarderS3Buckets: ""             ## ENTER A COMMA DELIMITED LIST OF S3 BUCKETS ARNS THAT ARE THE SOURCES OF THE S3 SQS EVENT NOTIFICATIONS (IF ANY).
----
+
...
----
  ...
  parameters = {
      ...
      ElasticServerlessForwarderSSMSecrets           = ""  ## ENTER A COMMA DELIMITED LIST OF AWS SSM SECRETS ARNS REFERENCED IN THE CONFIG YAML FILE (IF ANY).
      ElasticServerlessForwarderKMSKeys              = ""  ## ENTER A COMMA DELIMITED LIST OF AWS KMS KEYS ARNS TO BE USED FOR DECRYPTING AWS SSM SECRETS REFERENCED IN THE CONFIG YAML FILE (IF ANY).
      ElasticServerlessForwarderSQSEvents            = ""  ## ENTER A COMMA DELIMITED LIST OF DIRECT SQS QUEUES ARNS TO SET AS EVENT TRIGGERS FOR THE LAMBDA REFERENCED IN THE CONFIG YAML FILE (IF ANY).
      ElasticServerlessForwarderS3SQSEvents          = ""  ## ENTER A COMMA DELIMITED LIST OF S3 SQS EVENT NOTIFICATIONS ARNS TO SET AS EVENT TRIGGERS FOR THE LAMBDA REFERENCED IN THE CONFIG YAML FILE (IF ANY).
      ElasticServerlessForwarderKinesisEvents        = ""  ## ENTER A COMMA DELIMITED LIST OF KINESIS DATA STREAM ARNS TO SET AS EVENT TRIGGERS FOR THE LAMBDA REFERENCED IN THE CONFIG YAML FILE (IF ANY).
      ElasticServerlessForwarderCloudWatchLogsEvents = ""  ## ENTER A COMMA DELIMITED LIST OF CLOUDWATCH LOGS LOG GROUPS ARNS TO SET SUBSCRIPTION FILTERS ON THE LAMBDA REFERENCED IN THE CONFIG YAML FILE(IF ANY).
      ElasticServerlessForwarderS3Buckets            = ""  ## ENTER A COMMA DELIMITED LIST OF S3 BUCKETS ARNS THAT ARE THE SOURCES OF THE S3 SQS EVENT NOTIFICATIONS (IF ANY).
