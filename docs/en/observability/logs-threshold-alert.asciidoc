...
By using a ratio configuration it's possible to be alerted when a ratio value meets a certain threshold. These configurations are useful when you want to know how one query (Query A) compares to the other query (Query B). The ratio threshold value is the result of the document count of Query A divided by the document count of Query B.

The example below would trigger when there are more than twice as many error logs compared to warning logs.
