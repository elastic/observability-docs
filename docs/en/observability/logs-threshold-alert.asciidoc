...
When setting a group by we highly recommend using the "more than" comparator for your threshold if possible. This allows our queries to apply eager filtering, which can lead to significant performance improvements. If this isn't possible we suggest using a group by field with the lowest cardinality (number of possibilities) possible.
