...
Each of these comparators is backed by a different Elasticsearch query type, and in some scenarios it is important to know what these are so that you can configure your alert correctly. The comparators listed above map to the following Elasticsearch query types:
