...
If your project is not already in a version controlled directory add it now,
...
+
If any of your journeys fail, the synthetics agent it will yield a non-zero exit code,
which most CI systems pick up as a failure by default.
