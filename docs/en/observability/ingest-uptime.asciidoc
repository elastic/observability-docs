...
For further redundancy, you may want to deploy multiple Heartbeats across geographic and network boundaries
to provide more data. To do so, specify {heartbeat}'s observer {heartbeat-ref}/configuration-observer-options.html[geo options].

For example:

* A site served from a content delivery network (CDN) with points of presence (POPs) around the globe.
+
To check if your site is reachable via CDN POPS, you may want to have multiple {heartbeat} instances at
...
immediately apparent if the service had a connectivity issue to the outside world, or if the failure was only internal.
