...
[NOTE]
=====
If you haven't already, please install {es} for storing and searching your data, and {kib} for visualizing and
...
{heartbeat} is typically installed as part of a monitoring service that runs on a separate machine 
and possibly even outside of the network where the services that you want to monitor are running.

Use the commands that work with your system to download and install {beatname_uc}:
...
Each monitor is configured individually. In `heartbeat.yml`, specify the
...
- type: tcp
  ...
  mode: any <3>
----------------------------------------------------------------------
<1> The `icmp` monitor is scheduled to run exactly every five seconds (10:00:00,
...
<2> The `tcp` monitor is set to run every five seconds from the time when {beatname_uc}
...
{beatname_uc} can be deployed in multiple locations to detect 
...
Confirm your data is correctly ingested to your cluster.
