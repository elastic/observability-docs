...
NOTE: To run synthetics test, you must use the *elastic-agent-complete* Docker container as this contains the dependencies to run synthetic monitors.
...
Add the *elastic-agent-complete* to Fleet by running the Docker command below. Be sure to substitue the correct `FLEET_URL` and `FLEET_ENROLLMENT_TOKEN`
