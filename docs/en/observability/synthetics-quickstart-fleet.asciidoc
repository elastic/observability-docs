...
++++
<titleabbrev>Get started using {agent} and {fleet}</titleabbrev>
++++
...
Synthetics Browser checks can only run as the non-root `elastic-agent` user  in the `elastic-agent` container. We are currently working to simplify the permissions requirements to work with a single user. This can be tracked in https://github.com/elastic/beats/issues/27920[this github issue].
