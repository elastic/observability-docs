...
Set the *Source Type* for the browser monitor.

Use the *Zip URL* source type if you're using a <<synthetics-test-suite, synthetic test suite>>.
...
You can use the *Inline script* source type if you're using a single journey that doesn't rely on any dependencies.
