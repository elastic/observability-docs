...
Inline monitors can be more efficient, but have some limitations like not supporting dependencies or multiple journeys.
...
Inline monitors are quick and easy to setup.
To create one, you can copy a journey's content, paste it into Kibana or your {heartbeat} config, and hit save.
...
If you depend on external packages, have your journeys next to your code repo, or just wish to embed and manage multiple journeys from a single monitor configuration <<synthetics-test-suite,use a test suite>>.
...
If you have a suite of tests to implement, you can use the `@elastic/synthetics` package as a library.
