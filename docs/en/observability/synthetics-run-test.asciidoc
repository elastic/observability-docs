...
To see an example of a test suite, see the https://github.com/elastic/synthetics-demo[elastic/synthetics-demo] repository.
...
When writing a inline monitor, the `journey` keyword isn't required, and variables like `page` and `params` will be part of your script's scope.
To create an inline monitor, you can copy the steps from a journey, paste it into {kib} or your {heartbeat} configuration, and save.
