...
You can specfiy a default value for `schedule` and `locations` for all journeys in a global Synthetics configuration file.
...
beta[] Add a project when configuring the Elastic Synthetics integration.
