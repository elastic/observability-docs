...
You must configure the monitor for each journey, which allows you to set unique options for each monitor.
For example:
...
You can specify a default value for `schedule` and `locations` to be used for all journeys in a global Synthetics configuration file.
