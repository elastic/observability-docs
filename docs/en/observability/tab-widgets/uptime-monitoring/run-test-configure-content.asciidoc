...
Where to deploy the monitor when using <<private-locations,private locations>>. Monitors can be deployed in multiple locations including both `locations` and `privateLocations` so that you can detect differences in availability and response times across those locations.
