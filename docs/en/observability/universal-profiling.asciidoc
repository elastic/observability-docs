...
Elastic Universal Profiling is a continuous profiling tool running on top of the {stack}.

For a quick overview of Universal Profiling, see the https://www.elastic.co/observability/ebpf-continuous-code-profiling[Universal Profiling product page]
...
Adding symbols for unsymbolized frames is currently a manual operation. See *LINK NEEDED*.
...
sampling frequency of 20 Hz, will be stored in {es} at the rate of 20 MB per core per day.
