...
{filebeat} sends events to an existing {es} deployment, if present.
...
After the dashboards are loaded, navigate to dashboards for the applications
you're running. For example, navigate to the dashboards for MySQL or NGINX.
