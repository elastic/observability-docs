...
* {observability-guide}/getting-started-apm-server.html[Download and install APM Server] -- The classic, non-Kubernetes option.
====

[discrete]
=== Step 2: Save your secret token

A {observability-guide}/secret-token.html[secret token] is used to secure communication between APM agents
...
see {observability-guide}/secret-token.html[secret token] for instructions on how to set up your secret token.
