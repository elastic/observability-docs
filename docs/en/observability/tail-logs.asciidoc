...
servers, virtual machines, and containers in a centralized view. Consider this as a `tail -f` in your browser,
...
the workflow of monitoring logs, the icons next to each field value allow you to filter the logs per that value.
