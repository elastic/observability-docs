...
[float]
<<<<<<< HEAD:docs/en/observability/apm-ui/troubleshooting.asciidoc
[[no-apm-data-found]]
=== No APM data found
...
>>>>>>> 27fc737c ([apm] Update APM docs to reflect changes related to the Elasticsearch apm-data plugin (#4333)):docs/en/observability/apm/troubleshooting/common-problems.asciidoc
