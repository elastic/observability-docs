...
You’ll start with installing the Elastic GCP integration to include prebuilt
...
See below how to get your *Cloud ID* and generate an *API Key* that you'll
need in the next steps. 
====

You can find the Cloud ID of your https://cloud.elastic.co/deployments[deployment]
at its *Overview* page.

image:monitor-gcp-cloud-id.png[Cloud ID]

And you can use {kibana-ref}/api-keys.html#create-api-key[{kib}] to create a
...
[NOTE]
====
The *Cloud Pub/Sub subscription* is the same you created in the previous step.
The *Cloud ID* and *Base64-encoded API Key* are the same from first step.
If you don't have an *Error output topic*, you can created one like in the
previous step.
====
