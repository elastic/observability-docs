...
=== Upstream OpenTelemetry language SDKs and Collectors
...
==== Send data from an upstream OpenTelemetry language SDK
...
Define environment variables to configure the OpenTelemetry language SDK and enable communication with Elastic APM.
...
Some language implementations of the OpenTelemetry SDK, namely Python, requires the content of the header to be URL encoded. For the `Bearer` token that would mean just substituting the space between `Bearer` an the token with `%20`, e.g. `"Authorization=Bearer%20an_apm_secret_token"`, for an API key you can use the following snippet to handle that:
