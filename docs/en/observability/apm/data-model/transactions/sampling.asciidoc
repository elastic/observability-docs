...
[float]
[[apm-tail-based-sampling]]
== Tail-based sampling status_badge::[serverless, unavailable]
...
* status_badge::[serverless, unavailable] **Tail-based sampling**: Metrics are calculated based on all events, regardless of whether they are ultimately sampled or not.

* status_badge::[serverless, unavailable] **Both head and tail-based sampling**: When both methods are used together, metrics are calculated based on all events that were sampled by the head-based sampling policy.
...
== Configure tail-based sampling status_badge::[serverless, unavailable]
