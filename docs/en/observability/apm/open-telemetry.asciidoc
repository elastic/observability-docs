...
* <<apm-otel-upstream,Upstream OpenTelemetry language SDK or Collector>>
...
* preview:["The Elastic Distribution for OpenTelemetry Python is not yet recommended for production use. Functionality may be changed or removed in future releases. Alpha releases are not subject to the support SLA of official GA features."] https://github.com/elastic/elastic-otel-python[*Elastic Distribution for OpenTelemetry Python →*]
...
[float]
[[apm-otel-upstream]]
== Upstream OpenTelemetry language SDK or Collector
...
OpenTelemetry language SDK or OpenTelemetry Collector can be sent directly to the {stack}.
...
you can use an upstream OpenTelemetry language SDK to send data directly to APM Server.
...
Alternatively, you can use an OpenTelemetry Collector to collect data and send it directly to APM Server without maintaining multiple agents.

// Why you would _not_ choose this approach
However, there are some limitations when using language SDKs and collectors built and maintained
...
<<apm-open-telemetry-direct,*Get started with upstream OpenTelemetry SDKs and Collectors →*>>
