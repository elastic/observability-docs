...
These configuration options are only relevant to APM Server binary users. Fleet-managed users should see the {fleet-guide}/fleet-settings.html[Fleet output settings].
****
...
* <<apm-ssl-common-config,Common configuration options>>
* <<apm-ssl-client-config,Client configuration options>>
* <<apm-ssl-server-config,Server configuration options>>
...
When this option is configured, the <<apm-client-key,`key`>> option is also required.
...
When this option is configured, the <<apm-server-key,`key`>> option is also required.
The certificate option support embedding of the certificate:
