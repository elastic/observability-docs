...
This section describes common problems you might encounter when using {apm-server-or-mis} and the Applications UI in {kib}.
...
[float]
[[apm-no-data-indexed]]
== No data is indexed status_badge::[cloud_serverless, unavailable]
...
[float]
[[apm-common-ssl-problems]]
== Common SSL-related problems status_badge::[cloud_serverless, unavailable]
...
[[apm-io-timeout]]
[float]
== I/O Timeout status_badge::[cloud_serverless, unavailable]
...
In the agent logs, you won't see a sign of failures as the {apm-server-or-mis} asynchronously sends the data it received from the agents to {es}. However, the {apm-server-or-mis} and {es} log a warning like this:
...
[[apm-tail-based-sampling-memory-disk-io]]
[float]
== Tail-based sampling causing high system memory usage and high disk IO status_badge::[cloud_serverless, unavailable]
