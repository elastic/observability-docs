...
// Assuming the APM integration is no longer responsible for index templates and ingest pipelines,
// what is the fix now?
...
// Assuming the APM integration is no longer responsible for index mappings,
// what is the fix now?
...
The recommended index templates for APM come from the {es} apm-data plugin.
