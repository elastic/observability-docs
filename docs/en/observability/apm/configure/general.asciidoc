...
[apm-max_concurrent_decoders]]
[float]
=== Maximum concurrent decoders

Sets the limit on the number of concurrent batches
that can be decoded at one time. This effectively limits the amount of
memory consumed by the processors decoding the incoming intake events.
If not set or set to zero, it will be automatically tuned to provide
reasonable performance based on the host memory limits. 
If set manually, it should be set to a value that will not exceed the host
memory or may result in Out of Memory errors.

For {fleet}-managed users, this is not configurable.

|====
| APM Server binary | `apm-server.max_concurrent_decoders`
| Fleet-managed     | Not available
|====
