...
<<apm-secure-agent-communication,authenticated communication>> is disabled.
...
For this reason, anonymous auth will be enabled automatically if <<apm-apm-rum-enable>>
is set to `true`, and <<apm-config-auth-anon-enabled>> is not explicitly defined.

See <<apm-configuration-rum>> for additional RUM configuration options.
...
Use the <<apm-config-auth-anon-allow-agent>> and <<apm-config-auth-anon-allow-service>> configs to ensure that the
...
(`client.ip`) of the request with <<apm-config-auth-anon-event-limit>>.
