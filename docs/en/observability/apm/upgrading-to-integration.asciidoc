...
See <<apm-configuring-howto-apm-server>> for a reference of all available settings.
...
Once data from upgraded APM agents is visible in the Applications UI,
...
//QUESTION: Not sure how to handle the following instruction since the Settings are hidden under the app and can't be searched for. A user has to select one of the pages under Application, but not all of the pages have the Settings link. It's weird. Do you think I should change this or just leave it as-is?
...
Within minutes your data should begin appearing in the Applications UI again.
