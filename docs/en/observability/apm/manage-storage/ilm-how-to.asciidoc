...
// Should we mention DSL here, too?
// Should we establish the relationship between ILM and DSL?
...
// Should we link to https://www.elastic.co/guide/en/elasticsearch/reference/current/data-stream-lifecycle.html ?
...
// Is this right?
The table below describes the default data stream lifecycle policy applied to each APM data stream.
Each policy includes a rollover and delete definition:

// Does anything here need to be updated?
// For example:
// From "Using rollover indices prevents a single index from growing too large..."
// to "Using rollover data streams prevents a single data stream from growing too large..."
...
// Is this right?
// Does where they go in the UI change?
...
// Is this right?
TIP: Default data stream lifecycle policies can change between minor versions.
