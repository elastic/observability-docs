...
WARNING: Support for Jaeger is deprecated and will be removed in a future version from Elastic APM. https://www.jaegertracing.io/docs/1.35/client-libraries/[Jaeger clients are deprecated] in favour of OpenTelemetry SDKs and OpenTelemetry has removed all Jaeger exporters from their https://github.com/open-telemetry/opentelemetry-specification/pull/2858[specification].
...
Sampling decisions can be configured <<apm-configure-sampling-central-jaeger,centrally>> with {apm-agent} central configuration, or <<apm-configure-sampling-local-jaeger,locally>> in each Jaeger client.
...
IMPORTANT: There are <<apm-caveats-jaeger,caveats>> to this integration.
...
* <<apm-configure-sampling-central-jaeger,{apm-agent} central configuration (default)>>
* <<apm-configure-sampling-local-jaeger,Local sampling in each Jaeger client>>
...
See <<apm-agent-configuration,Central configuration>> to learn more.
...
** Server Jaeger spans are mapped to Elastic APM <<apm-data-model-transactions>>.
** Client Jaeger spans are mapped to Elastic APM <<apm-data-model-spans>> -- unless the span is the root, in which case it is mapped to an Elastic APM <<apm-data-model-transactions>>.
