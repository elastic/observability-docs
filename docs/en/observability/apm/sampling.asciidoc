...
In addition to setting the sample rate, you can also specify what trace continuation strategy to use.
This can be especially helpful if:

* An Elastic-monitored service is receiving requests with `traceparent` headers from unmonitored or third-party monitored services.
* An Elastic-monitored service is publicly exposed, and does not want tracing data to possibly be spoofed by user requests.

The default continuation strategy is `continue`, which will behave similar to the examples above.

The example below uses the `restart` continuation strategy in which a new trace is started with every service.
// Is this true? Is the opposite true?
`Service A` initiates four transactions, and because it is a third-party monitored service none of the transactions will be sampled.
`Service B` and `Service C` are Elastic-monitored services with sample rates of `.5` (`50%`) and `1` (`100%`) respectively.
Because these are treated as separate traces when using the `restart` continuation strategy,
the sample rate for each service is applied, unlike the examples above.
...
The next example uses the `restart_external` continuation strategy in which a new
trace is only started if the previous service was not Elastic-monitored.
`Service A` is an Elastic-monitored service initiates four transactions with a sample rate of `.25` (`25%`).
// Is `Service B` part of the trace that starts in `Service A`?
Because `Service B` is a third-party monitored service that will be the end of the traces started in `Service A`.
`Service C` will initiate four transactions that which start new traces with a sample rate of `.5` (`50%`).
Because `Service D` is also Elastic-monitored service, the sample rates of `Service D` is ignored.
