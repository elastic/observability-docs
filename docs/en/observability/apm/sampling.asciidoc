...
The *`continue`* trace continuation strategy is the default and will behave similar to the examples above.

Use the *`restart_external`* trace continuation strategy on an Elastic-monitored service to start
a new trace if the previous service did not have a `traceparent` header. This can be helpful if a
transaction includes an Elastic-monitored service that is receiving requests from an unmonitored service.

In the example below, `Service A` is an Elastic-monitored service that initiates four transactions
...
In the example below, `Service A` and `Service B` are Elastic-monitored services that use the
