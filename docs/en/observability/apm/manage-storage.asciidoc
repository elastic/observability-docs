...
{agent} uses <<apm-apm-data-streams,data streams>> to store time series data across multiple indices.
...
or to <<apm-apm-tune-elasticsearch,tune data ingestion in {es}>>.
...
If you want to delete data for security or privacy reasons, see <<apm-apm-data-security>>.
