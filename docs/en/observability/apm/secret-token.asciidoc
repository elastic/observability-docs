...
. Make sure <<apm-agent-tls,TLS>> is enabled
. <<apm-create-secret-token,Create a secret token>>
. <<apm-configure-secret-token,Configure the secret token in your APM agents>>
...
* *Android agent*: {apm-android-ref}/configuration.html[`secretToken`]
