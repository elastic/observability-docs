...
// TO DO: Figure out if all mentions of APM Server also apply to the managed intake service.
// If yes, should the example use an API key instead of a secret token?
...
* Send data to Elastic from an upstream <<apm-connect-open-telemetry-collector,OpenTelemetry Collector>>
* Send data to Elastic from an upstream <<apm-instrument-apps-otel,OpenTelemetry language SDK>>
...
exporters:
  ...
  otlp: <4>
    # Elastic APM server or managed intake service https endpoint without the "https://" prefix
    endpoint: "${env:ELASTIC_APM_SERVER_ENDPOINT}" <5> <7>
    headers:
      # Elastic API key
      Authorization: "ApiKey ${env:ELASTIC_APM_API_KEY}" <6> <7>
...
<6> Credential for Elastic APM:
* <<apm-api-key,API key authorization>> (`Authorization: "ApiKey an_api_key"`)
* <<apm-secret-token,secret token authorization>> (`Authorization: "Bearer a_secret_token"`) status_badge::[cloud_serverless, unavailable]
...
<8>  To send OpenTelemetry logs to {stack} version 8.0+, declare a `logs` pipeline. preview:[]
...
Other methods, like using the https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/exporter/elasticsearchexporter[`elasticsearch` exporter] to send data directly to {es} will send data to Elastic,
