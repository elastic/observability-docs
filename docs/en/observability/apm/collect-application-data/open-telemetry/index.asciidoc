...
OpenTelemetry Collector or OpenTelemetry language SDK can be sent to Elastic.
...
data to the collector, and use the collector to process and export the data to {apm-server-or-mis}.

image::images/apm-otel-api-sdk-collector.png[]

[NOTE]
====
It's also possible to send data directly to {apm-server-or-mis} from an upstream OpenTelemetry SDK.
