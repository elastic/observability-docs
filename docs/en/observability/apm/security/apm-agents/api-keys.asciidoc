...
In addition, when creating an API key from the APM UI,
...
== Create an API key in the APM UI

The APM UI has a built-in workflow that you can use to easily create and view {apm-agent} API keys.
Only API keys created in the APM UI will show up here.
...
[role="screenshot"]
image::images/apm-ui-api-key.png[APM UI API key]
...
include::{observability-docs-root}/docs/en/observability/apm/advanced-setup/command-reference.asciidoc[tag=apikey-subcommands]
