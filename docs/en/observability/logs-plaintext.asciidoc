...
Create an ingest pipeline with a {ref}/dissect-processor.html[dissect processor] to extract structured ECS fields from your log messages. In your project, navigate to **Developer Tools** and using a command similar to the following example:
...
To view logs ingested by {filebeat}, go to *Discover* from the main menu and creating a data view based on the `filebeat-*` index pattern. Refer to {kibana-ref}/data-views.html[Create a data view] for more information.
