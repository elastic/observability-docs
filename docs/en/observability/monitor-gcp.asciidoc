...
After starting and connecting to the instance, download and install
{metricbeat}.
...
Configure and test the output ({es}). Use the {metricbeat} keystore to store
...
echo -n "<Your Cloud ID>" | ./metricbeat keystore add CLOUD_ID --stdin
----

To store metrics in {es} with minimal permissions, create an API key to send
data from {metricbeat} to {ess}.

Log into Kibana (you can do so from the Cloud Console without typing in any
permissions) and select *Management* -> *Dev Tools*. Then, send the following
request:
...
Next, edit the `modules.d/googlecloud.yml` file to configure which metrics to
collect.
[source,yml]
----
- module: googlecloud
...
Input and output are ready. Next, you will setup {metricbeat} index template,
dashboards and ML jobs. This may take a few minutes.
[source,bash]
----
...
[NOTE]
====
Setting up index templates, dashboards, and ML jobs needs more permissions than
the ones defined by the `api_key` configured earlier. In this case, use the
`elastic` super user.
====

Next, start {metricbeat} to collect the data.
[source,bash]
----
./metricbeat -e
----
