...
- Export GCP audit logs though Pub/Sub topics
...
Next, click on *Create Service Account*. Then, define the new service account
...
Click on *continue*, then skip granting users access to this service. Finally,
click on *done* and the service account will be ready to be used.
...
. Finally, go to Kibana and open the "[{metricbeat} GCP] Compute Overview"
dashboard.
+
...
Now that {metricbeat} is up and running you will configure {filebeat} to
...
data from {filebeat} to {ess}. Log into Kibana (you can do so from the Cloud
Console without typing in any permissions) and select *Management* -> *Dev
Tools*. Then, send the following request:
+
[source,console]
...
. Then, configure {filebeat} to output to {ess}. Edit the `filebeat.yml`
configuration file and add the following lines to the end of the file.
+
[source,yml]
----
...
In order to collect logs from Google Cloud Platform, use the
...
+
Then, click on *create sink* to set up the logs routing sink.
...
. Then, write the following configuration to the `modules.d/gcp.yml`.
+
[source,yml]
----
- module: gcp
...
. Next, start {filebeat} to collect the data.
+
[source,bash]
----
./filebeat -e
...
. Finally, go to Kibana and open the "[{filebeat} GCP] Audit"
dashboard.
+
