...
It's is an important part of any system that you build and want to monitor.
Being able to detect and fix root cause events quickly within an observable system should be a minimum requirement for any analyst.
...
single stack to unify your logs, infrastructure metrics, uptime data, application traces, user experience data, synthetics, and universal profiling.
...
[subs=attributes+]
include::diagrams/what-is-obs.asciidoc[]
...
You can then drill-down into the {logs-app} by clicking *Show log stream* to view a live stream of your logs and filter, pin, or highlight the data you need.
