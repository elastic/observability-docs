...
that includes the https://docs.elastic.co/en/integrations/osquery_manager.html[Osquery Manager]
...
one or more {agent}s. Osquery results are stored in {es}
...
`memory_used` to be a little more user friendly), the process ID (`pid`), and the
process path (`name`) are returned in descending order.
