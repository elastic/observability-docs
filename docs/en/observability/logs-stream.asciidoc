...
The `host.ip` field is part of the {ecs-ref}/ecs-reference.html[Elastic Common Schema (ECS)]. Through the ECS, the `host.ip` field is mapped as an {ref}/ip.html[`ip` field type]. `ip` field types allow range queries so you can filter logs by IP addresses in a specific range. You can also query `ip` field types using CIDR notation to find logs with IPs in a particular network or subnet.

This section shows you how to extract the `host.ip` fields from the following example logs and query based on those fields:
...
. Query your logs based on the `host.ip` field.
...
You can query your logs based on the `host.ip` field in different ways. Two of the more common ways are range queries and CIDR notation. 

[discrete]
[[logs-stream-ip-cidr]]
==== Query your logs using CIDR notation
...
==== Query logs using range queries
...
When you're ingesting a large batch of log data, a single malformed IP address can cause the entire batch to fail. You can prevent this by setting `ignore_malformed ` to `true`.

The following command updates the `host.ip` field to ignore malformed IPs using the {ref}/indices-put-mapping.html[update mapping API]:
