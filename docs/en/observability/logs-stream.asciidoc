...
In this guide, you'll learn how to take a log file or directory from your host and send it to Elasticsearch using an {agent} in standalone mode. From there, you'll learn how to extract relevant information from logs, and transform the data into a standardized format for further processing and analysis. 
...
{agent} uses data streams to store time series data across multiple indices while giving you a single named resource for requests.
...
- namespace: User-configurable arbitrary grouping. Useful for grouping data. The default value is `default`.
...
You can leverage data streams to filter by a specific data type (logs, metrics etc.), dataset (nginx.access, prometheus) or namespace. The following are examples of common queries pertaining to specific datatypes, datasets, or namespaces:
