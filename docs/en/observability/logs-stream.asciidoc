...
** sets the <<ignore-malformed, `ignore_malformed`>> flag to `true`, which means that if a log document contains a field with a value of the wrong type, and the mapping type for this field supports this flag, then the document will not be rejected
