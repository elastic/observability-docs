...
This section shows you how to get the most out of your log data after sending it to {es} by extracting structured fields from your unstructured log data. Extracting structured fields makes it easier to search, analyze, and filter your log data. 

Let's look at the following log example:
...
Use the following command to look at the document's details:
...
This section shows you how to extract the `@timestamp` field from the following example log:
...
[discrete]
[[logs-stream-ingest-pipeline]]
==== Extract the timestamp with an ingest pipeline

To extract the `@timestamp` field, use an ingest pipeline with a dissect processor and a date processor. Processors are executed in the order they are defined, so in the following example: 

. The {ref}/dissect-processor.html[dissect processor] extracts a structured field from your unstructured log message based on a pattern you set. In the following example, it extracts the timestamp to the `timestamp` field.
...
The following example creates an ingest pipeline with a dissect and a date processor:
...
. Set the following values for the dissect processor:
...
+
. Set the following for the date processor:
- `_ingest/pipeline/logs-generic-default` – `logs-generic-default` is the name of the pipeline. The name of your pipeline needs to match the name of your data stream. You'll set up your data stream in the next section. See {fleet-ref}/data-streams.html#data-streams-naming-scheme[data stream naming scheme] for more information.
...
Optionally, you can set the following values for the `date` processor:

- `timezone` – Sets the timezone to use when parsing the date.
- `target_field` – Sets the output field for the timestamp.
- `output_format` – Changes the format when writing the date to the timestamp field. Must be a valid java time pattern.

[discrete]
[[logs-stream-index-template]]
==== Configure your data stream with an index template

After creating your ingest pipeline, you need to index your log data to a data stream using an index template. 

The following command creates an index template:
...
- `index_patterns`– The index pattern needs to match your log data stream. Naming conventions for data streams are `<type>-<dataset>-<namespace>`. If you're using default values, your logs data stream is named `logs-generic-default`. Data that matches this pattern will go through your pipeline.
- `data_stream` – Enables data streams.
- `priority` – Index templates with higher priority take precedence over lower priority. If a data stream matches multiple index templates, the template with the higher priority is used. Built-in templates have a priority of `200`, so we recommend a priority higher than that.
- `index.default_pipeline` – The name of the ingest pipeline you created.

[discrete]
[[logs-stream-create-data-stream]]
==== Create your data stream

Create your data stream using the {fleet-ref}/data-streams.html#data-streams-naming-scheme[data stream naming scheme]. The name needs to match the name of your pipeline. For this example, we'll use the default log data stream name, `logs-generic-default`, and the example log:
...
Now look at the details using the following command:
