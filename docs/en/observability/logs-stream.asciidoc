...
This stored the document in `logs-test-default`. You can retrieve it again with the following search:
...
Elasticsearch by default indexes the `message` field. This means you can search for phrases like `WARN` or `Disk usage exceeds`, but the `message` field can't be used for sorting or filtering. A search for `WARN` could look as following and shows the document as result.
...
Your message, however, contains all of the potential fields below. Extracting these will allow you to do filtering and aggregations on these fields:
...
Before we had ingested the document but if we look closer at the resulting document the `@timestamp` field shows when you added the data to {es}, not when the log occurred.
...
- `pattern`– The pattern of the elements in your log data. The following pattern extracts the timestamp, `2023-08-08T13:45:12.123Z`, to the `@timestamp` field, while the rest of the message, `WARN 192.168.1.101 Disk usage exceeds 90%.`, stays in the `message` field. This works because dissect looks for the space as a separator defined by the pattern `%{timestamp} %{message}`.

[discrete]
[[logs-stream-simulate-api]]
=== Test your pipeline with the simulate pipeline API

You can test if your ingest pipeline works as expected with the {ref}/simulate-pipeline-api.html#ingest-verbose-param[simulate pipeline API]. This runs the pipeline without storing any documents. This is great to test your pipeline with different documents. Run this command to test your pipeline:
...
PUT _index_template/logs-example-default-template
{
  ...
  "composed_of": ["ecs@dynamic_templates"],
}
----
