...
Data streams store time series data across multiple indices while giving you a single named resource for requests.
...
. Set the name for your dataset data stream in the *Dataset name* field. The dataset name describes the data ingested and its structure. This field can contain anything that makes sense to signify the source of the data. The default value is `generic`.
...
With your {agent} and data streams set up, you can filter and search your log data. In {kib}, navigate to *Observability → Logs → Stream*, use the search bar to search for your `data_stream.dataset` and `data_stream.type`. 
...
For more information on the data stream naming scheme, see {fleet-guide}/data-streams.html#data-streams[Data streams].