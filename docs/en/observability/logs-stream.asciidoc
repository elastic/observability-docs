...
This stored the document in `logs-example-default`. You can retrieve it again with the following search:
...
- `_ingest/pipeline/logs-example-default` – The name of the pipeline,`logs-example-default`, we recommend that it matches the name of your data stream. You'll set up your data stream in the next section. See the {fleet-guide}/data-streams.html#data-streams-naming-scheme[data stream naming scheme] for more information.
...
Some info about the proposed component templates composing the index template above:

* *`logs-mappings`* - general mappings for logs data streams, including:
** disabling automatic date detection from `string` fields
** mappings for the {ecs-ref}/ecs-data_stream.html[`data_stream` ECS fields]
* *`logs-settings`* - general settings for logs data streams, including: 
** default lifecycle policy: rolling over when primary shard's size reaches 50 GB, or after 30 days
** default pipeline that:
*** sets a `@timestamp` if such is missing (using the ingest timestamp)
*** places a hook for the `logs@custom` pipeline- if a pipeline with this name is installed, it will be applied to logs ingested into this data stream
** sets the {ref}/ignore-malformed.html[`ignore_malformed`] flag to `true`, which means that if a log document contains a field with a value of the wrong type, and the mapping type for this field supports this flag, then the document will not be rejected
* *`logs@custom`* - a predefined component template that is not installed by default. Use this name to install a custom component template if you wish to override/extend any of the default mappings/settings
* *`ecs@dynamic_templates`* - dynamic templates that ensure automatic compliance of your data stream mappings with the {ecs-ref}/ecs-reference.html[Elastic Common Schema (ECS)]
