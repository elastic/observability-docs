...
After downloading and extracting the installation package, you're ready to install the {agent}. From the agent directory, run the the install command that corresponds with your system:
...
After installing the agent, you'll find the `elastic-agent.yml` in the location according to your system:
...
To configure your {agent}, replace the contents in the elastic-agent.yml file with the preceding configuration. Then, set the values for the following fields:
...
After updating your configuration file, you may need to restart the {agent} to see your log data. To do this:
