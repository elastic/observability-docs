...
{es} indexes the `message` field by default. This means you can search for phrases like `WARN` or `Disk usage exceeds`. For example, you can search for logs with `WARN` in the message using the following command:
...
While you can search for phrases in the `message` field, you can't use it to filter log data. Your message, however, contains all of the following potential fields you can extract and use to filter and aggregate your log data:
...
To extract the `@timestamp` field from the example log, use an ingest pipeline with a dissect processor. Ingest pipelines consist of a series of processors that perform common transformations on incoming documents before they are indexed. 

The {ref}/dissect-processor.html[dissect processor] is one of the available processors. It extracts structured fields from your unstructured log message based on a pattern you set. 
...
The {ref}/simulate-pipeline-api.html#ingest-verbose-param[simulate pipeline API] runs the ingest pipeline without storing any documents. This is great for verifying your pipeline works with multiple different documents. Run the following command to test your ingest pipeline with the simulate pipeline API.
...
You can see your document's using this command:
...
The results should look like the following showing the pipeline has extracted the `@timestamp` field:
...
Extracting the `host.ip` field lets you filter logs by host IP addresses, so you can focus on specific hosts that you’re having issues with or find disparities between hosts. 
...
You can query your logs based on the `host.ip` field in different ways, including with CIDR notation and range queries. 
...
By default, an ingest pipeline sends your log data to a single data stream. To simplify log data management, you can use a {ref}/reroute-processor.html[reroute processor] to route data from the generic data stream to a target data stream. For example, you might want to send high-severity logs to a specific data stream to help with categorization. 
