...
In this guide, you'll learn how to take a log file or directory from your host and send it to Elasticsearch using the {agent} in standalone mode. From there, you'll learn how to set up data streams to have more control and flexibility when querying and filtering your log data. 
...
For more information, see {ref}/data-streams.html[Data streams].

[discrete]
[[logs-stream-configure-datastreams]]
=== Update data streams

You can update data streams in the `elastic-agent.yml` file under *inputs*. For file location after installing the {agent}, refer to {fleet-guide}/installation-layout.html[installation layout]. 
