...
To extract the `@timestamp` field from the example log, use an ingest pipeline with a dissect processor. The {ref}/dissect-processor.html[dissect processor] extracts structured fields from your unstructured log message based the pattern you set. In the following example, the dissect processor extracts the timestamp to the `@timestamp` field. {es} can parse string timestamps that are in `yyyy-MM-dd'T'HH:mm:ss.SSSZ` and `yyyy-MM-dd` format into date fields. 

If your log timestamps are more complex or use a nonstandard format, you need a {ref}/date-processor.html[date processor] to put the timestamp into a date field. You can also use a date processor to set the timezone, change the target field, and change the output format of the timestamp.

This command to create an ingest pipeline with a dissect processor:
