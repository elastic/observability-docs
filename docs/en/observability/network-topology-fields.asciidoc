...
| keyword
| ECS ext.
| Device category — set explicitly in the {ls} collector config. The ingest pipeline infers it from `observer.sys_descr` as a fallback when unset.
...
NOTE: These are cumulative counters, not rates. To compute bits/sec, take the delta between two consecutive polls in your collector or use a {kib} scripted field.
