...
[TIP]
=====
Select a tag in the monitor list to display only those monitors the tag is assigned to.
This can be valuable if you have assigned tags based on which cloud provider the monitor
is hosted on.
=====
