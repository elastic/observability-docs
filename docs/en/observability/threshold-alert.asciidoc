...
[IMPORTANT]
=====
If you select *group alerts by* fields, but no documents contain data for the selected field(s) within the given time range, you can't determine the group(s). This is relevant for rules that trigger alerts when the condition includes `0`.
For example, if your rule triggers an alert when the document count includes `0`, an alert won't be triggered in this instance because no data for the field is not treated as `0`.
=====
