...
When you added the log to {es} in the previous section, the `@timestamp` field was added showing when the log was added, while the timestamp showing when the log occurred was in the unstructured `message` field:
...
To investigate issues effectively, extract the timestamp from the unstructured `message` field into the structured `@timestamp` field. This enables efficient filtering by date and time ranges.
Extract the `@timestamp` field by completing the following:
...
In addition to your ingest pipeline, use the index template you created in the <<logs-stream-index-template, Extract the `@timestamp` field>> section.
