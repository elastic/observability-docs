...
In this {tutorial}, you'll learn how to monitor your Azure deployments using
...
This ID is required to configure {beats} to connect to your Azure account.
+
...
Then, click on Add to save the new key, which displays the key value in the
Value field. Copy the value and save it (along with your Client ID) for
future reference.
+
[IMPORTANT]
====
...
permission. You need `Reader` permission to configure {beats} to monitor your
services.
...
. In the **Select** field, paste the description name or application (client) ID
obtained when creating the Azure service principal.
+
...
To monitor Azure using the {stack}, you need two main components:
...
----
+
{metricbeat} will print {azure} metrics to the terminal, if the setup is correct.
...
. Finally, log into {kib} and open the *[{metricbeat} Azure] [Metricbeat Azure]
...
====
+
You can also check the **[Metricbeat Azure] Billing overview** dashboard.
+
...
. Run the following command to create a Event Hub namespace:
+
[source,bash]
----
...
default value suggest on the Azure quick start.
====

[discrete]
==== Configure activity logs to stream to event the hub
...
[source,bash]
----
./filebeat keystore add AZURE_CONNECTION_STRING
----
...
. Click on **View keys** and copy one of the keys.
+
...
[source,bash]
----
./filebeat keystore add AZURE_STORAGE_KEY
----
