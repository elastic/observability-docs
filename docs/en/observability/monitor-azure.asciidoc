...
- Ingest logs using the native integration and view those logs in Kibana.
...
in which you want to deploy {es}. You can read more about Azure
subscriptions in the https://docs.microsoft.com/en-us/azure/cost-management-billing/manage/add-change-subscription-administrator#assign-a-subscription-administrator[Microsoft Azure documentation].
====

. Search and click on *{es}*.
+
image:monitor-azure-search-elasticsearch.png[Search and click on {es}]

. Click on *Create*.

. Fill in the *Subscription*, the *Resource group*, and the *Resource name*.
Select a region and click on *Review + create*.
+
image:monitor-azure-create-elastic-resource.png[Create Elastic resource]
+
[NOTE]
====
*Logs & metrics* will be discussed later.
====

. Click on *Create* to create the {es} cluster.

. After your deployment is complete, click on *Go to resource*.
Here you can see and configure your deployment details.
Click on *{kib}* to access the cluster.
+
image:monitor-azure-elastic-deployment.png[Elastic resource]

. Select your Azure account to follow through with single sign-on directly
into Elastic.

. Click on *Observability* to see if there is any data available.
There should be no data yet, but you will ingest logs next.
+
...
It is very easy to ingest Azure subscription and resource logs into Elastic
using the Microsoft Azure native integration.

. Go back to the Elasticsearch resource page in Azure and click on
...
[NOTE]
====
This configuration can also be done during the Elastic resource creation.
This tutorial separates the two steps to make the concepts clearer.
====
+
[NOTE]
====
Native metrics collection is not fully supported yet and will be discussed
later.
====

. Go back to {kib} and click on *Observability* at the breadcrumbs until you
see some data.
It might take a few minutes before you see some data.
+
image:monitor-azure-kibana-observability-page-data.png[{kib} observability page
(with data)]
 
. Click *View in app* to access the Logs app and analyze all your subscription
and resource logs.
+
...
. Go to your Elastic resource and click on *Virtual machines*.
+
image:monitor-azure-elastic-deployment.png[Elastic resource]

. Select the VMs that you want to collect logs and metrics from, click on
*Install Extension*, and click *OK*.
+
image:monitor-azure-elastic-vms.png[Select VMs to collect logs and metrics from]

. Wait until it is installed and sending data (click on *Refresh* if the list
does not update).
Then, click on *Logs* to see the VMs logs in the Logs app.
+
image:monitor-azure-kibana-vms-logs.png[VMs logs in the Logs app]
+
And click on *Metrics* to see the VMs metrics dashboard.
+
image:monitor-azure-kibana-vms-metrics.png[VMs metrics dashboard]
+
[NOTE]
====
Both logs and metrics will be filtered by the VM's name that you clicked on.
Delete the filter to view the data for all the monitored VMs.
====
...
// {metricbeat-ref}/metricbeat-module-azure.html[{metricbeat} Azure module] and view those metrics in {kib}.
...
To properly configure {metricbeat} you will need the Elasticsearch cluster
details.

. Go to the Elasticsearch resource page and click on *Manage changes in
Elastic Cloud*.
+
image:monitor-azure-elastic-deployment.png[Elastic resource]

. Copy the *Cloud ID* and keep it safe as you will use it later.
+
image:monitor-azure-kibana-deployment.png[Elastic Cloud deployment]

. Click on *Security* and then on *Reset password*. Confirm, copy the
password, and keep it safe as you will use it later.
+
image:monitor-azure-kibana-security.png[Elastic Cloud security]

You can run the {metricbeat} on any machine. This {tutorial} uses a small
...
dashboard. (Keep in mind it collects data every 24h.)
+
