...
Redis or MySQL. A module consists of metric sets that fetch and structure the data. To learn more, see
...
Use the commands that work with your system to download and install {metricbeat}:
...
. Identify the modules that need to be enabled. To see a list of available {metricbeat-ref}/metricbeat-modules.html[modules], run:
+
--
include::{beats-repo-dir}/tab-widgets/list-modules-widget.asciidoc[]
--

. Enable one or more modules from the installation directory. If you accept the default configuration without enabling additional
