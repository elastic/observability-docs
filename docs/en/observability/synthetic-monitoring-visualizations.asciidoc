...
Based on the Uptime data you are sending to {ess}, you can create various visualizations
...
. Select the monitor name and the URL. You can make multiple selections.
...
+
You can filter by `observer location`, `monitor type`, and any `tags` you previously created for a *pings* report.
...
+
For a *pings* report, you can segment data by `observer location` or `monitor type`.
