...
The second method, uses the `zip_url` source type, which points to remote endpoint where synthetics tests can be downloaded with basic authentication.
...
remote zip endpoint which is https://github.com/elastic/synthetics/tree/master/examples/todos[our todos example]. 
<4> Note that the url refers to the endpoint where the test projects exists.
<5> folder refers to the relative path where the synthetic journey files are
located. {heartbeat} will invoke the synthetics library on this folder.
<6> Username and password if provided will be sent as HTTP Basic Authentication headers to the remote zip endpoint.
