...
From Discover, use KQL in the search bar to filter for logs with within your timestamp range with log levels of `WARN` and `ERROR`:
...
Make sure your logs fall within the time range in Discover. If necessary, update the time range by clicking the image:images/time-filter-icon.png[calendar icon, width=36px]. 

Under the *Documents* tab, you'll see the log data matching your query. 
You can then expand the document to see more information on the log. 
