...
. Set the `hosts` to your deployment's {es} endpoint. Copy the {es} endpoint from your deployment's page and add the port (the default port is `443`). For example, `<YOUR_ELASTICSEARCH_ENDPOINT>:443`.
+
--
[role="screenshot"]
