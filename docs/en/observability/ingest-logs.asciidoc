...
Install and configure {filebeat} on your servers to collect log events. {filebeat} allows you to ship log data from sources that come
...
. Identify the modules that need to be enabled. To see a list of available {filebeat-ref}/filebeat-modules.html[modules], run:
+
--
...
. Enable one or more modules form the installation directory. For example, the
...
Next, confirm your data is correctly streaming to your cloud instance.
