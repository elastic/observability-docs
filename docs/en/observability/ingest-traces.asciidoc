...
****
As {ecloud} spins up an {agent} instance automatically or 
self-managed users installed an {agent} instance manually in Step 1, 
this step is optional only for users who need to add a new Elastic Agents.
If you only need to use APM features, {ess} and self-managed users both should skip this step.
****
