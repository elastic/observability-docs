...
= Running on self hosted Elastic stack

++++
<titleabbrev>Self-hosted infrastructure</titleabbrev>
++++

This section describes how to run Universal Profiling when hosting the Elastic stack on your own infrastructure.

beta::[]

Universal Profiling is made of a client part (the host agent) and a server part (the backend): this document focuses on the server part.
So far, the backend has only been available on {ecloud}, but we are also making it available for self hosted Elastic stack users.
...
We don't support running the Universal Profiling backend in ECK, but you can still connect the backend to an Elastic stack managed through ECK.

[[profiling-self-managed-backend-support-matrix]]
Find here a list of platforms where we have tested running successfully the Universal Profiling backend.

.Expand to see the entries.
[%collapsible]
====
|====
| Platform | OS | Details
...
[discrete]
[[profiling-sefl-managed-architecture-overview]]
== Architecture overview

The backend is made of two services: collector and symbolizer.

Collector s responsible for receiving profiling data from the host agents, and ingesting it into Elasticsearch.
It listens on an HTTP server and serves a gRPC endpoint.

Symbolizer is responsible for processing debug symbols that are not available on the host agent, and for symbolizing native frames from OS packages.
...
The installation process is composed of:

. Update the Elastic stack to the minimum supported version.
. Enable Universal Profiling in Kibana.
. Set up Universal Profiling in Kibana.
. Run the backend applications.

If during the installation you face any issues, please refer to <<profiling-self-managed-troubleshooting,"Troubleshooting Universal Profiling backed">>.

After the installation fo Universal Profiling is complete, please refer to <<profiling-self-managed-ops,"Operating the Universal Profiling backend">> for more details on how to monitor and scale the backend.

Find below the instructions to update the stack on each supported platform.

The procedure for the third step is equal to all platforms, and is described in <<profiling-self-managed-setup-kibana,"Setting up Universal Profiling via Kibana">>.

The last step is to run the backend applications, and the procedure is described in <<profiling-self-managed-running-linux,"Running on Linux">> and <<profiling-self-managed-running-kubernetes,"Running on Kubernetes">>.
You don't need to execute this step when deploying on Universal Profiling on ECE, because it is handled automatically for you.
...
Update ECE to version 3.7.0 or higher, following the steps in the https://www.elastic.co/downloads/enterprise[installations instructions].

[discrete]
==== Self-managed Elastic stack

WARNING: We don't yet support running the backend applications through Elastic Agent.
...
We don't support running the _backend_ applications through ECK just yet, but if you are running an Elastic stack through ECK you can still connect the Universal Profiling backend applications to it.
...
Kibana needs to be configured to enable the Universal Profiling plugin.

The relevant piece of configuration is the following:
...
See how to do it in the https://www.elastic.co/guide/en/cloud-enterprise/current/ece-manage-elastic-stack.html#ece-manage-elastic-stack[documentation].

[discrete]
==== Self-managed Elastic stack

. Edit the Kibana YAML configuration file, usually named `kibana.yml`, adding the config line mentioned above.
. Restart Kibana to relad the configuration

[discrete]
==== Kubernetes

If you're using ECK, add the above configuration line to the `kibana.k8s.elastic.co/v1` CRD, placing it under the `spec.config` key.
Read more about how to configure Kibana in the https://www.elastic.co/guide/en/cloud-on-k8s/current/k8s-kibana-advanced-configuration.html#k8s-kibana-configuration[ECK documentation].

If you're not using ECK, edit the `secret` or `configMap` holding the `kibana.yml` configuration file.
Add the config line mentioned above, then perform a rolling restart of the Kibana deployment to reload the configuration.

[[profiling-self-managed-setup-kibana]]
=== Set up Universal Profiling in Kibana

Follow the steps described in <<profiling-configure-data-ingestion,"Configure data ingestion">> to execute the setup of Universal Profiling.

**IF you are running on ECE**, you can stop here, the setup is complete, because the backend applications are automatically deployed for you via the Integrations Server.

**If you are not running on ECE**, go ahead reading to finalize the setup of Universal Profiling backend.
...
Both collector and symbolizer need to authenticate to Elasticsearch to process profiling data.
...
Here is the role definition that includes the required permissions for the API keys.
...
You will need these API keys to run the Universal Profiling backend on Linux or Kubernetes.

[[profiling-self-managed-running-linux]]
==== Running on Linux

There are three options to install the backend on Linux:
...
For any option you may choose, creating a configuration file for the backend applications is the recommended (but not necessary) way to manage them.
...
The "application" section contains the configuration for the backend applications, and the "output" section contains the configuration to connect to where the data will be read abd sent to.
...
Customize the content of the `output.elasticsearch` section, using the Elasticsearch endpoint and API key created above to set the `hosts` and `api_key` values, respectively.
...
===== Configuration file for symbolizer
...
[[profiling-self-managed-install-linux]]
===== Run the backend applications

You can now proceed to choose one of the options below to run the Universal Profiling backend applications.

[[profiling-self-managed-install-linux-packages]]
[discrete]
====== DEB packages
...
After completing the installation of packages, enable and start the systemd services:
...
Refer to <<profiling-self-managed-troubleshooting,"Troubleshooting Universal Profiling backed">> for more information on how to troubleshoot possible errors spotted in the logs.
...
With the config file in places in your system, you can run the containers with the following commands (the example command uses Docker, but any OCI runtime will work):
...
NOTE: In the examples above we used the charts' default values to configure Kubernetes resources. These **do not** include the creation of an `Ingress` resource.
...
Congratulations! Everything is in place now.

You can follow the steps described in <<profiling-install-host-agent,"Install the host agent">> to install the host agent on a machine, and verify that the backend is working as expected.
...
If you spot problems, refer to <<profiling-self-managed-troubleshooting,"Troubleshooting Universal Profiling backend">> for more information on how to troubleshoot possible errors spotted in the logs.
