...
If data streams are specified, hints can be defined for each one. For example, `co.elastic.hints/key.period: 10m` if the data stream specified is `info` from the {metricbeat-ref}/metricbeat-module-redis.html[Redis module]. Otherwise, the data stream will use the top-level hints
