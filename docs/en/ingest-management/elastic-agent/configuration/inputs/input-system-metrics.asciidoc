...
Settings that affect the behavior of specific metricsets.
...
Settings that control the metrics collected by the `system/metrics` input.

{agent} provides the following metricsets. Some of these metricsets have
additional settings as described in the following sections.
...
`core.metrics`::
(list) Controls what metrics are reported for each CPU core. Valid metric types
...
(string) Periodof time to cache the results of failed reverse lookups.
+
*Default:* `60s`
