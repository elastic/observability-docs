...
The following Logstash configuration file configures a {ls} pipeline that
listens on port `5044` for incoming {agent} connections and routes received
events to {es}:
...
//QUESTION: How are data stream names passed to Logstash? I'm
//assuming that happens automatically and that users don't need to configure a
//setting like index. Is that correct?
...
|
[id="output-{output-type}-proxy_use_local_resolver-setting"]
`proxy_use_`
`local_resolver`
...
//QUESTION: Is this list complete? Does LS only support ssl options? If so, I
//wonder if it's even worthing having this section here.
...
| Configuration options for SSL parameters like the root certificate
authority (CA) for {ls} connections.
...
|
[id="output-{output-type}-loadbalance-setting"]
`loadbalance`
...
|
[id="output-{output-type}-pipelining-setting"]
`pipelining`
...
|
[id="output-{output-type}-slow_start-setting"]
`slow_start`
...
// tag::ttl-setting[]
|
[id="output-{output-type}-ttl-setting"]
`ttl`
...
//QUESTION: Is the default for the logstash output 1? In Beats, the default is documented
//for Elasticsearch, but not for Logstash.
