...
| (string) The {kib} username used to enable {fleet}.
Overrides `KIBANA_USERNAME` when set.

// Todo: add permissions or link to permissions required for this action.
...
| (string) The {kib} password used to enable {fleet}.
...
| (string) The {es} username for {fleet-server}.
...
| (string) The {es} password for {fleet-server}.
