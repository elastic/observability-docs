...
- If used https to access kube-state-metrics, to all `kubernetes.state_*` dataset should be added:
[source,yaml]
-----
...
. If namespace, where elastic-agent is running, has `"openshift.io/node-selector"` annotation set, override node
selector for the namespace to allow scheduling on any node:
+
[source,shell]
----
