...
** `kubernetes.controllermanager` datastream condition should be change to:
+
[source,yaml]
------------------------------------------------
condition: ${kubernetes.labels.app} == 'kube-controller-manager'
------------------------------------------------
** `kubernetes.scheduler` datastream condition should be changed to:
+
[source,yaml]
------------------------------------------------
condition: ${kubernetes.labels.app} == 'openshift-kube-scheduler'
------------------------------------------------
** `kubernetes.proxy` datastream configutation should look like:
+
[source,yaml]
------------------------------------------------
- data_stream:
  ...
  period: 10s
------------------------------------------------
** to all datastreams that connnect to `https://${env.NODE_NAME}:10250` should be added:
+
[source,yaml]
------------------------------------------------
  bearer_token_file: /var/run/secrets/kubernetes.io/serviceaccount/token
...
for GCP then the following `configmap` can be mounted in elastic-agent pod and use `ca-bundle.crt`
...
. If namespace, where elastic-agent is running, has `"openshift.io/node-selector"` annotation set, elastic-agent
might not run on all nodes. In this case consider overriding node selector for the namespace to allow scheduling
on any node:
+
[source,shell]
----
