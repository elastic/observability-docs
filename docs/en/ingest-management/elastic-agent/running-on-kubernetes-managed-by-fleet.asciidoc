...
The {agent} needs to be assigned to a policy to enable the proper inputs. To achieve Kubernetes observability, the policy would need to have the Kubernetes integration. Refer to <<create-a-policy>> and <<add-integration>> to learn how to enable the https://docs.elastic.co/en/integrations/kubernetes[Kubernetes integration].
