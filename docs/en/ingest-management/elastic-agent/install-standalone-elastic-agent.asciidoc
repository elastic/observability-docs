...
+
To get started quickly and avoid errors, use {kib} to generate a standalone
configuration file rather than trying to build it by hand. For more information,
refer to <<generate-standalone-agent-config>>.
+
...
//QUESTION: What privileges are required for this user? Do we need to add
//index privileges on sythetics-* and traces-*. What else? Is there an existing
//role that would work instead of asking the user to create a role?
