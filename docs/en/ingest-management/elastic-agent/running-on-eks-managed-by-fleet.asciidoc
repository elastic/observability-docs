...
+
In this regard, you can use https://www.elastic.co/guide/en/beats/filebeat/current/add-fields.html[`add_fields` processor] to set `orchestrator.cluster.name` and `orchestrator.cluster.url` fields in order to add those fields for each https://docs.elastic.co/en/integrations/kubernetes[Kubernetes integration]'s component:
[source,yaml]
.Processors
------------------------------------------------
  - add_fields:
