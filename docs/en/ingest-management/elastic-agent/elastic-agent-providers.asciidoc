...
Providers supply the key and value pairs that are used for variable substitution
...
`{"key1": "value1", "key2": "value2"}`, the key and value pairs are placed in
...
They can only provide a single key and value mapping. Think of them as singletons;
an update of a key and value mapping results in a re-evaluation of the entire
configuration. These providers are normally very static, but not
required. A value can change which results in re-evaluation. 

Context providers use ECS naming to ensure consistency and understanding throughout documentation and projects. 
