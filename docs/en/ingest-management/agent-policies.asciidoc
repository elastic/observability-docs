...
Once the policy is saved, the configuration on these agents will be modified
to use the new {fleet-server} as the controller.

Make sure that the {agent}s in this policy all have connectivity to the {fleet-server}
adding the {fleet-server} to the policy. Lack of connectivity will prevent the {agent}
