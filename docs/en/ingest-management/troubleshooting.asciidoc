...
----
Note: Port 443 from the example above is typical with use in {ecloud}, but with self-managed deployments your {es} may be running on port 9200 or something else entirely.
...
[discrete]
[[ca-cert-testing]]
== On Fleet Server start up, ERROR seen with 'State changed to CRASHED: exited with code: 1'

This error message can be shown for different reasons but a common one, when attempting production-like usage,
is seen when the ca.crt file passed in cannot be found.  One way to verify if this is the problem you are
seeing is to bootstrap {fleet-server} without passing a ca.crt file, which implies you would test any subsequent
{agent} installs temporarily with {fleet-sever}'s own self-signed cert.

Tip: be sure to pass in the full path to the ca.crt file, a relative path is not viable.

Further, you will know your {fleet-server} is set up with its testing oriented self-signed certificate usage
if you see the following error during {agent} installs:
