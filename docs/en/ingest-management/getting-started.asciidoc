...
{agent}s manually can <<install-standalone-elastic-agent,run agents standalone>>.
...
NOTE: To create a *browser* monitor, you must use the elastic-agent-complete Docker container as this contains the dependencies to run browser monitors. To learn more, see {observability-guide}/synthetics-quickstart-fleet.html[Synthetic monitoring via {agent} and {fleet}].
