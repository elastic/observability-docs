...
== Step 4: Monitor HTTP, TCP, ICMP, and Browser services
[discrete]
...
| *Browser* | Runs synthetic monitoring tests via Synthetic Agent on the Chromium browser.

For detailed information about Browser options, see our
{heartbeat-ref}/monitor-browser-options.html[Heartbeat] documentation.

NOTE: To create a "Browser" monitor, you must use "complete" Elastic Agent Docker container as this contains the dependencies to run browser monitors. For more information, see the {observability-guide}/synthetics-quickstart-fleet.html[Synthetic monitoring via Elastic Agent and Fleet quickstart guide].
...
. To view the data, click *View all agent policies* to return to the *{fleet}* home page,
then click the **Data streams** tab.
