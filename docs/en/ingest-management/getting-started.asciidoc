...
Next, you’ll add the Elastic Synthetics integration, enabling you to monitor the
...
. On the **Add Elastic Synthetics** page, select the `default` policy.
+
****
...
. Under **Configure integration**, enter the monitor name and select *HTTP* from the following monitor types:
+
|=== 
...
. Enter the monitor URL and select a monitor interval in seconds or minutes. By default, a monitoring
