...
Use the search bar to find the Elastic Synthetics integration.
+
[role="screenshot"]
...
enter the integration name and select *HTTP* from the following monitor types:
+
|===
...
and use the search bar to find the Nginx integration.
+
[role="screenshot"]
