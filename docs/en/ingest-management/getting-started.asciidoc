...
* An {es} cluster and {kib} (version 7.9) with a basic license.
...
. On the **Add integration** page, click the default Agent configuration
...
that the **System** integration is installed by default.
+
[role="screenshot"]
...
the default config.
+
The newly created integration configuration should appear on the **Integrations** tab.
Note that the `system-1` integration configuration has been created by default.
+
[role="screenshot"]
