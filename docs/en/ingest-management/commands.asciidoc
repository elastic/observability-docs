...
the `--insecure` flag.
<2> SHA256 fingerprint of the CA that {es} generates for self-signed certificates.
...
The hash of {es}'s certificate authority that will used by
{fleet-server} and any inputs the {agent} starts for communication.
Used specifically to verify self-signed certificates.
