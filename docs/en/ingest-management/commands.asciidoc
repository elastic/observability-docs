...
Note: if the {agent} is already installed on the host, using --force will result in
unpredictable behaviour with duplicate {agent}s appearing in {fleet}

`--non-interactive`::
The {agent} will be installed in a non-interactive mode. This flag is helpful when 
using automation software or scripted deployments and will check if there is an {agent}
already installed. If there is an {agent} on the host already, installation will abort.
