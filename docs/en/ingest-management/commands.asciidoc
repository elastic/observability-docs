...
If `pprof` is specified, it will gather specific `pprof` data from the {agent} or one of the specified underlying {beats} and display to `stdout`.
By default it will gather a 30s profile of the {agent}.
...
Or output file name for the `pprof` option.
Defaults to `elastic-agent-diagnostics-<timestamp>.zip` where the timestamp is the current time in UTC when running `collect` or `stdout` when running `pprof`.
