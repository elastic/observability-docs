...
`--fleet-server-policy <string>`::
Used when starting a {fleet-server} process to allow for options configured the
specified policy. 
...
[source,shell]
----
elastic-agent enroll -f --url https://other-host:8220 \
  --enrollment-token NEFmVllaa0JLRXhKebVKVTR5TTI6N2JaVlJpSGpScmV0ZUVnZVlRUExFQQ==
...
[source,shell]
----
elastic-agent install -f --url https://other-host:8220 \
  --enrollment-token NEFmVllaa0JLRXhKebVKVTR5TTI6N2JaVlJpSGpScmV0ZUVnZVlRUExFQQ==
  --certificate-authorities root.ca
----
