...
agent as a service. You might want to do this if you installed
...
<<elastic-agent-install-command,`install` command>> instead.

We recommend that you run the `enroll` command as the root user because some
...
//TODO: Test this option with the enroll command to verify it works.
...
`enroll` command instead of `install`. The DEB and RPM packages include a
service unit for Linux systems with systemd.
