...
The aseets for a given integration can be re-loaded into {es} with the API.
You'll need to know the name of the package and the version you want to reinstall.
Use cURL or your favorite API UI tool to execute the following call with the noted "force" parameter
in the body:
