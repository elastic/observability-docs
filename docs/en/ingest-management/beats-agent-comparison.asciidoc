...
|{n}
|{n}
|{fleet}-managed {agent}s require root permission, in particular for {elastic-defend}. Standalone {agent}s will also require root permissions. {beats} requires root permission only if it's configured to capture data that requires that level of permission.
