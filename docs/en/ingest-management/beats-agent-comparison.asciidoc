...
(GA) on {agent}. To find out if an integration is GA, go to
{integrations-docs}[{integrations}] and navigate to the docs for the
integration. If there is no label, such as Beta or Experimental, the integration
is GA.
