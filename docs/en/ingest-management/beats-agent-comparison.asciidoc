...
**Step 3:** Review https://www.elastic.co/guide/en/fleet/current/beats-agent-comparison.html#additional-capabilties-beats-and-agent[Supported Capabilities] to determine if any features required by your deployment are supported. {agent} should support most of the features available on {beats} and is updated for each release.
