[[decode_xml_wineventlog-processor]]
...
| No
| `true`
| A boolean that specifies whether to map additional ECS fields when possible. Note that ECS field keys are placed outside of `target_field`.

| `ignore_missing`
| No
| `false`
| If `true` the processor will not return an error if a specified field does not exist.
