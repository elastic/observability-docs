...
| No
| `auto`
| The syslog format to use: `rfc3164` or `rfc5424`. To automatically detect the format from the log entries, set this option to `auto`.
...
| No
| `true`
| A boolean that specifies whether keys that already exist in the event are overwritten by keys from the syslog message.
...
//REVIEWERs: Removed the section about internal metrics returned by the HTTP
//endpoint because I don't think this info is relevant when using integrations
//and elastic agent, but let me know if I need to add it back in. 
