...
{fleet-server} is a component in the {stack} used to centrally manage {agent}s.
It is launched as part of an {agent} on a host intended to act as this server.
One {fleet-server} process can support many Agents {agent} connections.  Further,
{fleet-server} serves as a control plane for updating agent policies, collecting
