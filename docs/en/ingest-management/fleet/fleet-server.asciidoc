...
{fleet-server} is a component of the {stack} used to centrally manage
<<elastic-agent-installation-configuration,{agent}s>>. It's launched as part of
an {agent} on a host intended to act as a server. One {fleet-server} process can
support many {agent} connections, and serves as a control plane for updating
...
//Nima: Should we say something about hosting on ECE here?
...
//Nima: I've moved the detailed info about allocating resources and setting
//availability zones to the section about scaling because I want to make sure
//skim readers don't miss this step.
...
//Nima: These images have a gray background. I think they would look better with
//a white background, but maybe this is good enough for the initial release?
