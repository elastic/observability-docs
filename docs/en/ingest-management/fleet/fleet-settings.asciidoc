...
In cases where each client has a unique certificate, the path to that certificate can be
placed here instead of the certificate itself.
...
In cases where each client has a unique certificate key, the path to that certificate key can be
placed here instead of the certificate key itself.
