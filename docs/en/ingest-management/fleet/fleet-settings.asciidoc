...
{agent}s must be able to access the Elastic artifacts repository to download
binaries during upgrades. By default {agent}s download artifacts from the
Elastic artifacts repository at `https://artifacts.elastic.co/downloads/`.
...
To add or edit a binary download location:
...
repository.
//TODO: Make sure this is correct ^^. Might be that users need to specify path to a tarball.
//REVIEWERS: The text in the UI describes the Host setting as follows, but to me,
//it sounds like we are telling users to set the source for *each* binary:
// "Address that your agents will use to download their binary
//from. Specify the path to the directory containing the binary." I think what
//we want is just the address where the artifacts are hosted, right?
