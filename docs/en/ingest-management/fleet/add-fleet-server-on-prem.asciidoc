...
Before setting up {fleet-server}, you will need a
...
. Update the {es} hosts in the **Outputs** table:
.. Find the _default_ row where the _Type_ set to _Elasticsearch_.
...
select an existing one. Alternatively you can{fleet-guide}/create-a-policy-no-ui.html[create a {fleet-server} policy without using the UI],
