...
//REVIEWERS: From the design doc, it seemed like scheduled upgrades and
//maintenance windows are only available to some subscription levels. But looking
//at a snapshot build, seemed like that might not be true. Can you clarify?
//If the upgrade procedure is different for Platinum than other users, I guess
//I can break this into two sections: "Upgrade to latest version" and something like
//"Rolling upgrades" 
