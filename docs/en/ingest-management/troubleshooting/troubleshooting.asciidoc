...
==  The {agent} is stuck in status `updating`

The {agengt} can hang up in some rare situations in the `updating` status during the upgrade process. You can clean those agents with a forceful unenrollment as authenticated `superuser`.
This command will remove the stale {agent}. 
