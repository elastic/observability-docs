...
This error occurs when you use self-signed certificates with {es} using IP as a Common Name. 
With IP as a CN {fleet-server} looks into alternative names which is empty. To work around this situation
you need to use `--fleet-server-es-insecure` flag which disables certificate verification.
