...
Run the following command to generate a a zip archive containing diagnostics information that can be usable by the Elastic team for debugging cases.
...
. beats (and other process) version numbers and process meta-data.
. local configuration, elastic-agent policy, and config that is rendered and passed to beats and other processes.
...
out the integration update to additional hosts.

After <<update-integration,updating the integration>>:
