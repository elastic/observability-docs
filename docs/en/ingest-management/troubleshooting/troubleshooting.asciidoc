...
== {agent}s hosted on {ecloud} are stuck in `Updating` or go `Offline`
...
. Clean the existing `.fleet-policies` doc. In the Dev Tools Console, run:
+
[source,console]
----
...
[source,shell]
----
curl '<kibana_url>/api/fleet/agents/<agentID>/unenroll' \ <1>
  -u <username>:<password> \
  -H 'kbn-xsrf: xx' \
  ...
  --compressed
----
<1> `<agentID>` is the ID you copied in the previous step.
