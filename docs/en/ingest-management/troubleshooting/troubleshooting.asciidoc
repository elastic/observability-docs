...
|https://github.com/elastic/beats/issues[elastic/beats] | {agent} or {beats} shippers
...
By default this command will take a little more than 30s to run as it will collect 30s trace and profile information from the agent and all underlying beats.
The duration of the trace and profile data can be changed with the `--pprof-duration` flag.

The timeout value for the collection command can also be set with the `--timeout` flag, it will default to 30s + `pprof-duration`.
This flag should be increased in cases where the diagnostics command times out before creating the archive.
...
include::{fleet-repo-dir}/release-notes/release-notes-7.14.asciidoc[tag=fleet-server-input-missing-detail]

include::{fleet-repo-dir}/release-notes/release-notes-7.14.asciidoc[tag=fleet-server-input-missing-fix]
