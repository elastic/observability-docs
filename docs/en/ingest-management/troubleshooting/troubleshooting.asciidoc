...
The {agengt} can hung up in some rare situations in the `updating` status. This can be cleaned as authenticated `superuser`.
After this the {agent} will return the real status again.
