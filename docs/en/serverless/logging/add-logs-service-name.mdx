...
The **log error %** metric  show the rate of error logs per minute for a given `service.name`.
To accurately measure this metric, you need to ensure your services' logs provide the `log.level`.
In cases where the `log.level` is embedded in the log message, you may need to extract that information into the correct field.
