...
* <<logs-filter,Filter logs>>: Narrow down your log data by applying specific criteria.
* <<logs-aggregate,Aggregate logs>>: Analyze and summarize data to find patterns and gain insight.
...
:role: Admin
:goal: create ingest pipelines and set the index template
include::../partials/roles.asciidoc[]
:role!:

:goal!:

The examples on this page use the following ingest pipeline and index template, which you can set in **Developer Tools**. If you haven't used ingest pipelines and index templates to parse your log data and extract structured fields yet, start with the <<parse-log-data,Parse and organize logs>> documentation.
...
* <<logs-filter-logs-explorer,Filter logs in Logs Explorer>>: Filter and visualize log data in Logs Explorer.
* <<logs-filter-qdsl,Filter logs with Query DSL>>: Filter log data from Developer Tools using Query DSL.
...
From Logs Explorer, you can use the {kibana-ref}/kuery-query.html[{kib} Query Language (KQL)] in the search bar to narrow down the log data that's displayed.
