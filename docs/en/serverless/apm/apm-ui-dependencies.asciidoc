...
you can utilize <<apm-trace-sample-timeline-distributed-tracing,distributed tracing>> to drill down into problematic downstream services.
...
Selecting an operation displays the operation's impact and performance trends over time, via key metrics like latency, throughput, and failed transaction rate. In addition, the <<apm-trace-sample-timeline,**Trace sample timeline**>> provides a visual drill-down into an end-to-end trace sample.
