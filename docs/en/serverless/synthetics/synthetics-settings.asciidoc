...
* In the UI when you <<synthetics-get-started-ui,create a monitor>>.
* In the UI _after_ a monitor is already created, on the **Monitors** page
or on the **Edit monitor** page for the monitor.
* In a Synthetics project when <<synthetics-lightweight,configuring a lightweight monitor>>.
====
...
Read more about all available connectors in <<create-anomaly-alert-rule,Action types>>.
...
To learn more about the other methods and which methods take precedence over others, see <<synthetics-params-secrets,Work with params and secrets>>.
...
Learn more about the data included in each data stream in <<synthetics-manage-retention,Manage data retention>>.
...
Learn more about using API keys in <<synthetics-get-started-project,Create monitors with a Synthetics project>>.

[IMPORTANT]
====
To create a Project API key, you must be logged in as a user with
<<synthetics-feature-roles,Editor>> access.
====
