...
. From the data set table, click {icon-expand} next to a data set with poor or degraded quality.
...
Under the **actions** column, you'll find the degraded document icon ({icon-indexClose}).

Now that you know which documents contain ignored fields, examine them more closely to find the origin of the issue:

. Under the **actions** column, click {icon-expand} to open the document details.
. Select the **JSON** tab.
