...
{/* QUESTION: Is this correct? Is an Admin-level role required to do this? */}

During Technical Preview,
the Elastic Entity Model is enabled when a you opt into using the entity-centric service inventory described in <DocLink slug="/serverless/observability/new-experience-services" />.

{/* QUESTION: What if the user who opts in doesn't have sufficient privileges? Can they still opt in and ask someone else to enable EEM?
If so, is there a command the admin can run, or do they also need to opt-in? */}
...
<DocCallOut title="Notes">
    ...
    - During Technical Preview, Entity Definition Framework components are not enabled by default
</DocCallOut>

{/* QUESTION: Should this say "Entity Definition Framework" or "Entity Discovery Framework"? */}
