...
`untracked`::
The corresponding rule is disabled or you've marked the alert as untracked. To mark the alert as untracked, go to the **Alerts** table, click the {icon-boxesHorizontal} icon to expand the _More actions_ menu, and click **Mark as untracked**.
...
Click the {icon-boxesHorizontal} icon to add the alert to a new or existing case.
