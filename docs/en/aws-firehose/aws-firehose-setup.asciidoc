...
In {kib}, navigate to *Management > Integrations* in the sidebar.
Enable *Display beta integrations* first and then find the *Amazon Kinesis Data Firehose* integration by searching or browsing the catalog.

[role="screenshot"]
image::images/firehose-integration-page.png[Integrations catalogue with the "Firehose" integration highlighted]

Click *Add Amazon Kinesis Data Firehose* on this page.

[role="screenshot"]
image::images/firehose-integration-add.png[Firehose integration page with the "Add" button highlighted]
--
...
[WARNING]
====
The default data stream will change to `logs-awsfirehose-default` in the near future.
Please use `es_datastream_name` parameter with `logs-awsfirehose-default` to avoid breaking change.
====
