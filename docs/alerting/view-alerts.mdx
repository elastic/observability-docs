...
An alert has an `active` status when the conditions defined in the rule are met.
An alert has a `recovered` status when the rule conditions are no longer met.
...
A look back window and threshold are used to determine whether alerts are flapping. For example, in **((observability))** -> **Alerts** -> **Settings** you can specify that the alert must change status at least 6 times in the last 10 runs. If the rule has actions that run when the alert status changes, those actions are suppressed while the alert is flapping.
</DocCallOut>
