...
SLO burn rate breaches occur when the percentage of bad events over a specified time period exceeds the threshold set in your <DocLink id="serverlessObservabilityCreateSloBurnRateAlertRule" text="SLO burn rate rule"/>.
...
2. From the Alerts table, click the <DocIcon type="boxesHorizontal" size="m" title="More actions icon" />
...
* Click the <DocIcon type="boxesVertical" size="m" title="Actions" /> icon and select **Add to case** to add the alert to a new or existing case. To learn more, refer to <DocLink id="serverlessObservabilityCases"/>.
